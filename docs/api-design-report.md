...
1. 构造 `LayoutInput`，并以 UTF-16 offset 表示所有 source range；
...
| --- | --- | --- | --- | --- |
| 文本容器 | `Text` | source text 的 UTF-16 友好表示 | 所有样本文本 | 不是 Rust UTF-8 byte offset 模型 |
| source range | `TextRange` | span、注释、选择、glyph 对应范围 | 高度频繁 | offset 单位是 UTF-16 code unit |
...
| 行/范围几何 | `get_line_for_offset`、`get_bounding_box`、`get_bounding_boxes` | selection、link rect、overlay | range 按 UTF-16 offset；可切分多 code-unit cluster |
| caret 与 hit-test | `get_cursor_rect`、`get_offset_for_position` | 编辑器交互 | 普通位置定位 |
| 安全选择 | `get_selection_offset_for_position`、`coerce_selection_offset`、word boundary 查询 | 编辑器选择 | emoji ZWJ、surrogate、combining sequence 不从中间切开 |
...
### UTF-16 是全 API 的 source-coordinate 标准

`TextRange`、`LineBox.range`、span、glyph cluster range、查询 offset 和 fixture wire 都使用 UTF-16 code unit，而 Rust `str` 本身采用 UTF-8 byte offset。demo 的 `range_of` 显式通过 `encode_utf16().count()` 转换。这是与 Kotlin 镜像一致的正确约束，但必须成为对外 API 的第一等文档和类型命名要点。

风险在于：`TextRange::new` 只检查 `start <= end` 且非负，不能保证 offset 落在文本中或位于 Unicode 安全边界；部分完整性检查在 `layout` 调用时才通过 `assert!` 执行。集成层应定义何时验证、错误如何返回，以及是否提供从 Rust byte/char/grapheme 位置转换的官方工具。
...
1. 将宿主文本位置统一转换为 UTF-16 offset，构造所有 `TextRange`。
