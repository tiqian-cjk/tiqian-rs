...
fn kinsoku_rule_allows_clusters_without_display_text() {
    let empty = Cluster::new(
        text_range(0, 0),
        Text::new(),
        FontFaceId::with_resource_id("stub"),
        0.0,
    );
...
fn bopomofo_parser_covers_every_tone_arm() {
    ...
    assert_eq!(
        ...
        bopomofo_parser::parse(&Text::from("ㄅㄚˊ")).tone
    );
    assert_eq!(
        BopomofoTone::Shang,
        bopomofo_parser::parse(&Text::from("ㄅㄚˇ")).tone
    );
    assert_eq!(
        BopomofoTone::Qu,
        bopomofo_parser::parse(&Text::from("ㄅㄚˋ")).tone
    );
    assert_eq!(
        BopomofoTone::Yinping,
        bopomofo_parser::parse(&Text::from("ㄅㄚˉ")).tone
    );
    ...
    assert_eq!(
        vec![Text::from("ㄅ"), Text::from("ㄚ"), Text::from("ˈ")],
        in_range_default.symbols
    );
    assert_eq!(
        BopomofoTone::Yinping,
        bopomofo_parser::parse(&Text::from("ㄅㄚˌ")).tone
    );
}
