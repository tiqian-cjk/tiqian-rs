...
use tiqian::clreq::clreq_profile::{
    ...
    CjkPunctuationGlyphPolicy, ClreqProfile, ClreqProfileResolver, ClreqRegion, ClreqStrictness,
    GlueSide, HangingPunctuationStyle, InteriorPunctuationStyle, KinsokuLevel, KinsokuMode,
    LineAdjustmentStrategy, LineEndPunctuationStyle, PunctuationClass, PunctuationGluePlacement,
    PunctuationWidthPolicy, clreq_punctuation_advance_policy, clreq_punctuation_policies,
};
...
fn bopomofo_models_and_parser() {
    let reading = BopomofoReading {
        symbols: vec![Text::from("ㄅ"), Text::from("ㄚ")],
    ...
    assert_eq!(
        ...
        bopomofo_parser::parse(&Text::new()).tone
    );
    for (text, tone) in [
        ("˙ㄇㄚ", BopomofoTone::Neutral),
        ("ㄇㄚˊ", BopomofoTone::Yangping),
        ("ㄇㄚˇ", BopomofoTone::Shang),
        ("ㄇㄚˋ", BopomofoTone::Qu),
        ("ㄇㄚˉ", BopomofoTone::Yinping),
        ("ㄇㄚ", BopomofoTone::Yinping),
    ] {
        assert_eq!(
            tone,
            bopomofo_parser::parse(&Text::from(text)).tone,
            "{text}"
        );
    }
}
...
fn clreq_profile_and_resolver() {
    assert!(
        ClreqProfile::mainland_horizontal()
            .coalesce_repeatable_punctuation
            .contains(&0x2014)
    );
    assert_eq!(
        "clreq-mainland-horizontal",
        ClreqProfile::mainland_horizontal().id
    );
    assert_eq!(
        "clreq-taiwan-horizontal",
        ClreqProfile::taiwan_horizontal().id
    );
    assert_eq!(
        "clreq-hongkong-horizontal",
        ClreqProfile::hong_kong_horizontal().id
    );
    assert_eq!(
    ...
    for id in [
        ...
        LayoutProfileId {
            value: "clreq-mainland-horizontal".into(),
        },
        LayoutProfileId {
    ...
    assert_eq!(
        ...
        ClreqProfile::mainland_horizontal().strictness
    );
    assert_eq!(
        CjkPunctuationGlyphPolicy::PreferClreqRecommendedCodepoints,
        ClreqProfile::mainland_horizontal().punctuation_glyph_policy
    );
}
...
fn clreq_punctuation_policies_and_classification() {
    for character in [',', '.', ':', ';', '!', '?'] {
        assert!(clreq_punctuation_policies::is_ascii_point_mark(character));
    }
    for character in ['a', '，'] {
        assert!(!clreq_punctuation_policies::is_ascii_point_mark(character));
    }
    ...
    assert_eq!(
        ...
        clreq_punctuation_policies::classify('·')
    );
    for character in ['・', '‧', '•'] {
        assert_eq!(
    ...
    assert_eq!(
        ...
        clreq_punctuation_policies::classify('中')
    );
}

#[test]
fn forced_half_width_and_policy_for() {
    assert!(clreq_punctuation_policies::forced_half_width(
        '-',
    ...
    for character in ['～', '·', '•', '/'] {
        assert!(clreq_punctuation_policies::forced_half_width(character, gb));
    }
...
fn forbidden_at_line_start_and_end() {
    assert!(!clreq_punctuation_policies::forbidden_at_line_start(
        '，',
        KinsokuLevel::None
    ));
    assert!(!clreq_punctuation_policies::forbidden_at_line_end(
        '（',
        KinsokuLevel::None
    ));
    for character in ['，', '）', '～', '·', '•', '/'] {
...
fn punctuation_advance_and_substitutor() {
    ...
    assert_eq!(
        3.0,
        clreq_punctuation_advance_policy::advance_em(&Text::from("abc"), &Text::from("abc"))
    );
    assert_eq!(
        1.0,
        clreq_punctuation_advance_policy::advance_em(&Text::from("😀"), &Text::from("dummy"))
    );
    let force = tiqian::clreq::clreq_profile::ClreqPunctuationGlyphSubstitutor::new(
...
fn profile_builder_and_kinsoku_resolution_keep_region_and_measure_policy_explicit() {
    ...
    .kinsoku_mode(KinsokuMode::measure_adaptive_with_thresholds(
        10.0, 20.0, 30.0,
    ))
    .punctuation_width(PunctuationWidthPolicy::new(
        InteriorPunctuationStyle::Kaiming,
        true,
    ))
    .build();
    ...
    assert_eq!(
        ...
        profile
            .glue_placement
            .glue_side_for(PunctuationClass::Opening)
    );
    assert_eq!(
        LineAdjustmentStrategy::PushOutOnly,
        profile.adjustment.line_adjustment
    );
    ...
    let fixed =
        KinsokuMode::fixed_with_hanging(KinsokuLevel::None, HangingPunctuationStyle::PauseStops)
            .resolve(80.0);
