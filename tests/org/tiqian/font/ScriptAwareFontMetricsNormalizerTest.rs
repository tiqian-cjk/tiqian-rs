...
use tiqian::org::tiqian::font::FontPolicy::{
    FontMetricsPolicy, FontRole, RawFontMetrics,
};
...
fn cjk_text_uses_font_declared_typo_box_instead_of_synthesized_square() {
    ...
    let layout = ScriptAwareFontMetricsNormalizer.normalize(&FontMetricsNormalizationInput { request, raw_metrics: raw.clone() });
...
fn cjk_text_falls_back_to_hhea_when_font_has_no_typo_metrics() {
    ...
    let layout = ScriptAwareFontMetricsNormalizer.normalize(&FontMetricsNormalizationInput { request, raw_metrics: raw });
