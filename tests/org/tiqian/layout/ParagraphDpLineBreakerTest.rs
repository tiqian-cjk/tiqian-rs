...
fn assert_tiles(solution: &tiqian::org::tiqian::layout::LineOptimization::LineSolution, cluster_count: i32) {
    let mut expected = 0;
    for line in &solution.lines {
        if line.cluster_range.is_empty() {
            continue;
        }
        assert_eq!(expected, line.cluster_range.first(), "lines must tile clusters in order");
...
fn mandatory_break_binds_control_to_previous_line() {
    let clusters = vec![
        cluster(0, "中", 16.0), cluster(1, "中", 16.0), cluster(2, "\n", 0.0),
        cluster(3, "中", 16.0), cluster(4, "中", 16.0),
    ];
...
fn never_breaks_inside_unbreakable_range() {
    ...
    for line in &solution.lines {
        assert!(!(4..=6).contains(&line.cluster_range.first()), "break inside unbreakable range: {:?}", line.cluster_range);
    }
}
...
fn kinsoku_avoidance_routes_around_forbidden_line_start() {
    ...
    for line in &solution.lines {
        assert!(line.cluster_range.first() != 6 || line.repair.is_some(), "。 must not start a line without recorded repair");
    }
}
...
fn compression_edge_records_push_in_repair_only_when_enabled() {
    let clusters = vec![
        cluster(0, "中", 16.0), cluster(1, "中", 16.0), cluster(2, "中", 16.0),
        cluster(3, "，", 16.0), cluster(4, "中", 16.0), cluster(5, "中", 16.0), cluster(6, "中", 16.0),
    ];
    ...
    let line = compressed.lines.iter().find(|line| matches!(line.repair, Some(RepairOption::PushIn { .. }))).expect("expected a PushIn compressed line");
    assert_eq!(IntRange::new(0, 3), line.cluster_range);
    assert!(line.adjusted_width <= 56.01);
    let Some(RepairOption::PushIn { reason, .. }) = &line.repair else { unreachable!() };
...
fn overwide_single_cluster_still_progresses() {
    let clusters = vec![cluster(0, "中", 16.0), cluster(1, "Ｗ", 300.0), cluster(2, "中", 16.0)];
