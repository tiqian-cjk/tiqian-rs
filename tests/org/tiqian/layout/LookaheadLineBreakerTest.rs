...
fn cluster(start: i32, text: &str, advance: f32) -> Cluster {
    Cluster::new(TextRange::new(start, start + 1), text.to_owned(), "test".to_owned(), advance)
}

fn break_lines(breaker: &dyn LineBreaker, clusters: &[Cluster], max_width: f32, config: LineBreakerConfig) -> tiqian::org::tiqian::layout::LineOptimization::LineSolution {
...
fn hanging_tail_is_excluded_from_fill_density_geometry() {
    ...
    assert_eq!(0, tiqian::org::tiqian::layout::LineBreaker::line_gap_count(line.in_measure_cluster_range(), &HashSet::from([1, 2])));
    assert_eq!(0.0, tiqian::org::tiqian::layout::LineBreaker::line_adjustment_density(&line, 48.0, false, &HashSet::from([1, 2])));
}

#[test]
fn empty_input_produces_no_lines() {
    let solution = break_lines(&LookaheadLineBreaker::default(), &[], 100.0, LineBreakerConfig::default());
...
fn lookahead_matches_greedy_when_shifting_earlier_gives_no_benefit() {
    ...
    let solution = break_lines(&LookaheadLineBreaker::default(), &clusters, 64.0, LineBreakerConfig::default());
...
fn lookahead_shifts_break_earlier_to_avoid_kinsoku_repair() {
    let clusters = vec![
        cluster(0, "中", 16.0), cluster(1, "文", 16.0), cluster(2, "中", 16.0),
        cluster(3, "文", 16.0), cluster(4, "中", 16.0), cluster(5, "文", 16.0),
        cluster(6, "。", 16.0),
    ];
    let greedy = break_lines(&GreedyLineBreaker::default(), &clusters, 48.0, LineBreakerConfig::default());
    let lookahead = break_lines(&LookaheadLineBreaker::default(), &clusters, 48.0, LineBreakerConfig::default());

    assert!(matches!(greedy.lines[2].repair, Some(RepairOption::CarryPrevious { .. })));
    assert_eq!(10.0, greedy.total_badness);
    assert_eq!(vec![IntRange::new(0, 1), IntRange::new(2, 4), IntRange::new(5, 6)], lookahead.lines.iter().map(|line| line.cluster_range).collect::<Vec<_>>());
...
fn lookahead_scores_future_push_in_before_choosing_earlier_break() {
    ...
    config.shrink_opportunities = vec![ShrinkOpportunity::new(6, 6, 4.0, ShrinkChannel::TrailingGlue)];
    let solution = break_lines(&LookaheadLineBreaker::default(), &clusters, 60.0, config);

    assert_eq!(vec![IntRange::new(0, 2), IntRange::new(3, 6)], solution.lines.iter().map(|line| line.cluster_range).collect::<Vec<_>>());
    assert_eq!(60.0, solution.lines[1].adjusted_width);
    assert!(matches!(solution.lines[1].repair, Some(RepairOption::PushIn { .. })));
...
fn lookahead_scores_kinsoku_repairs_with_unbreakable_ranges() {
    ...
    assert_eq!(vec![IntRange::new(0, 1), IntRange::new(2, 5), IntRange::new(6, 8)], solution.lines.iter().map(|line| line.cluster_range).collect::<Vec<_>>());
...
fn window_zero_reduces_lookahead_to_greedy() {
    ...
    assert!(matches!(solution.lines[2].repair, Some(RepairOption::CarryPrevious { .. })));
}
