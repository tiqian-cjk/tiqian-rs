...
fn layout(
    ...
    ExplainableStubParagraphLayoutEngine::default().layout(
        LayoutInput::builder(
            TiqianTextContent::builder("中文".to_owned()).spans(spans).build(),
...
fn bopomofo_symbols_and_tones_occupy_right_side_annotation_zone() {
    let result = layout(
        vec![
            RubySpan::with_kind(TextRange::new(0, 1), "ㄓㄨㄥ".to_owned(), RubyKind::Bopomofo),
            RubySpan::with_kind(TextRange::new(1, 2), "ㄔㄤˊ".to_owned(), RubyKind::Bopomofo),
        ],
        Vec::new(),
    );
    let zhong = result.debug.bopomofo_decisions.iter().find(|decision| decision.base_range.start() == 0).unwrap();
    let chang = result.debug.bopomofo_decisions.iter().find(|decision| decision.base_range.start() == 1).unwrap();

    assert_eq!(700, zhong.font_weight);
    assert_eq!(3, zhong.placements.iter().filter(|placement| placement.role == BopomofoGlyphRole::Symbol).count());
    assert!(zhong.placements.iter().all(|placement| placement.left >= 15.9));
    assert_eq!(2, chang.placements.iter().filter(|placement| placement.role == BopomofoGlyphRole::Symbol).count());
    assert_eq!(1, chang.placements.iter().filter(|placement| placement.role == BopomofoGlyphRole::Tone).count());
}
...
fn bopomofo_reserves_annotated_base_without_changing_unannotated_neighbor() {
    ...
    let annotated = layout(
        vec![RubySpan::with_kind(TextRange::new(0, 1), "ㄓㄨㄥ".to_owned(), RubyKind::Bopomofo)],
        Vec::new(),
    );
...
fn bopomofo_weight_clamps_and_annotation_keeps_reading_locale_and_neutral_tone() {
    let weighted = layout(
        ...
        vec![
            TextSpan { range: TextRange::new(0, 1), style: TextStyle::builder().font_weight(500).build() },
            TextSpan { range: TextRange::new(1, 2), style: TextStyle::builder().font_weight(700).build() },
        ],
    );
    assert_eq!(800, weighted.debug.bopomofo_decisions.iter().find(|decision| decision.base_range == TextRange::new(0, 1)).unwrap().font_weight);
    assert_eq!(900, weighted.debug.bopomofo_decisions.iter().find(|decision| decision.base_range == TextRange::new(1, 2)).unwrap().font_weight);

    let neutral = layout(
        vec![RubySpan::with_kind(TextRange::new(0, 1), "˙ㄉㄜ".to_owned(), RubyKind::Bopomofo)],
        Vec::new(),
    );
