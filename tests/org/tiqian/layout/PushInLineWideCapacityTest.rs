...
fn cluster(index: i32, text: &str) -> Cluster {
    Cluster::new(TextRange::new(index, index + 1), text.to_owned(), "test".to_owned(), 16.0)
}
...
fn break_lines(clusters: &[Cluster], opportunities: Vec<ShrinkOpportunity>, max_width: f32) -> tiqian::org::tiqian::layout::LineOptimization::LineSolution {
...
fn push_in_aggregates_shrink_from_multiple_preceding_clusters() {
    ...
    assert_eq!(vec![10, 5], allocations.iter().map(|allocation| allocation.cluster_index).collect::<Vec<_>>());
    assert_eq!(vec![8.0, 8.0], allocations.iter().map(|allocation| allocation.shrink).collect::<Vec<_>>());
}

#[test]
fn push_in_rejects_when_line_wide_capacity_is_insufficient() {
    let clusters = punctuation_line();
    let solution = break_lines(&clusters, vec![ShrinkOpportunity::new(5, 6, 4.0, ShrinkChannel::TrailingGlue)], 160.0);
    ...
    let rejected = solution.lines[1].repair_candidates.iter().find(|candidate| candidate.kind == "PushIn").unwrap();
    assert!(!rejected.accepted);
    assert_eq!(Some("insufficient-capacity".to_owned()), rejected.rejection_reason);
...
fn offender_only_capacity_remains_supported() {
    let clusters = vec![cluster(0, "中"), cluster(1, "文"), cluster(2, "中"), cluster(3, "。")];
    let solution = break_lines(&clusters, vec![ShrinkOpportunity::new(3, 6, 4.0, ShrinkChannel::TrailingGlue)], 60.0);

    assert_eq!(1, solution.lines.len());
    let Some(RepairOption::PushIn { allocations, total_shrink, .. }) = &solution.lines[0].repair else { panic!("expected PushIn repair") };
...
fn push_in_merges_an_offender_that_fits_after_chained_repairs() {
    let clusters = vec![
        cluster(0, "中"), cluster(1, "中"), cluster(2, "中"), cluster(3, "」"), cluster(4, "。"),
        cluster(5, "中"), cluster(6, "中"), cluster(7, "中"), cluster(8, "、"), cluster(9, "中"),
    ];
    let solution = break_lines(
        &clusters,
        vec![
    ...
    assert!(solution.lines.iter().all(|line| clusters[line.cluster_range.first() as usize].text == "中"));
    let Some(RepairOption::PushIn { total_shrink, reason, .. }) = &solution.lines[1].repair else { panic!("expected zero-shrink PushIn repair") };
...
fn carry_previous_refuses_to_split_an_unbreakable_span() {
    let clusters = vec![
        cluster(0, "中"), cluster(1, "中"), cluster(2, "王"), cluster(3, "小"), cluster(4, "明"), cluster(5, "。"),
    ];
    let mut config = LineBreakerConfig::default();
    config.shrink_opportunities = vec![ShrinkOpportunity::new(5, 6, 8.0, ShrinkChannel::TrailingGlue)];
    ...
    assert_eq!(2, solution.lines.len());
    let Some(RepairOption::LeaveRagged { reason, .. }) = &solution.lines[1].repair else { panic!("expected LeaveRagged repair") };
    assert!(reason.ends_with("carry-would-split-mourning-span"));
    let carry = solution.lines[1].repair_candidates.iter().find(|candidate| candidate.kind == "CarryPrevious").unwrap();
    assert_eq!(Some("carry-would-split-mourning-span".to_owned()), carry.rejection_reason);
}
