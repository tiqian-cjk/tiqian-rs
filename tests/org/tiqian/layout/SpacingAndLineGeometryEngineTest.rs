...
fn layout(style: ParagraphStyle, text: &str, decorations: Vec<DecorationSpan>) -> tiqian::org::tiqian::core::LayoutModel::LayoutResult {
    ExplainableStubParagraphLayoutEngine::default().layout(
        LayoutInput::builder(TiqianTextContent::new(text.to_owned()), LayoutConstraints::with_defaults(240.0))
...
fn cjk_line_box_uses_font_declared_ideographic_typo_metrics() {
    let result = layout(ParagraphStyle::builder().first_line_indent(Some(Ic::ZERO)).build(), "提椠", Vec::new());
    let line = &result.lines[0];
    let cjk = result.debug.metric_decisions.iter().find(|decision| decision.role == "CjkText").unwrap();
    ...
    for height in [24.0, 48.0] {
        let result = layout(
            ParagraphStyle::builder()
                ...
                .build(),
            "着重",
            vec![DecorationSpan { range: TextRange::new(0, 2), kind: DecorationKind::Emphasis }],
        );
        let dot = result.debug.decoration_decisions.iter().find(|decision| decision.applied).unwrap();
        assert!((dot.anchor_y - (result.lines[0].baseline + 16.0 * 0.12 + 16.0 * 0.25 + dot.dot_diameter / 2.0)).abs() < 0.01);
    }
}

#[test]
fn interlinear_marks_clamp_tight_line_height_to_spacing_floor() {
    let marks = vec![DecorationSpan { range: TextRange::new(0, 4), kind: DecorationKind::Emphasis }];
    let clamped = layout(
        ParagraphStyle::builder().first_line_indent(Some(Ic::ZERO)).line_height(Some(20.0)).build(),
        "豆子新鲜",
        marks.clone(),
    );
    ...
    let generous = layout(
        ParagraphStyle::builder().first_line_indent(Some(Ic::ZERO)).line_height(Some(28.0)).build(),
        "豆子新鲜",
        marks,
    );
