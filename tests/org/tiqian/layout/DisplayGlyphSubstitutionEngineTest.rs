...
fn layout(text: &str, profile: Option<Box<dyn ClreqProfileResolver>>) -> tiqian::org::tiqian::core::LayoutModel::LayoutResult {
...
fn recommended_display_glyphs_preserve_source_text() {
    ...
    let ellipsis = result.clusters.iter().find(|cluster| cluster.text == "……").unwrap();
    let dash = result.clusters.iter().find(|cluster| cluster.text == "——").unwrap();
    let interpunct = result.clusters.iter().find(|cluster| cluster.text == "・").unwrap();
    let solidus = result.clusters.iter().find(|cluster| cluster.text == "／").unwrap();
    ...
    assert!(result.clusters.iter().all(|cluster| cluster.font_key == "cjk-primary"));
}
