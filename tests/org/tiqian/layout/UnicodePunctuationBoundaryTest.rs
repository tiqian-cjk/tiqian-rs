...
use tiqian::org::tiqian::clreq::ClreqProfile::{ClreqProfile, ClreqProfileResolver, KinsokuLevel, KinsokuMode};
...
fn layout_with_greedy(text: &str, max_width: f32) -> tiqian::org::tiqian::core::LayoutModel::LayoutResult {
    ...
    engine.layout(
        LayoutInput::builder(TiqianTextContent::new(text.to_owned()), LayoutConstraints::with_defaults(max_width))
...
fn line_texts<'a>(result: &tiqian::org::tiqian::core::LayoutModel::LayoutResult, text: &'a str) -> Vec<&'a str> {
    result
        .lines
        .iter()
        .map(|line| {
            let start = tiqian::org::tiqian::core::TextIndex::utf16_offset_to_utf8_byte_index(text, line.range.start()).unwrap();
            let end = tiqian::org::tiqian::core::TextIndex::utf16_offset_to_utf8_byte_index(text, line.range.end()).unwrap();
            &text[start..end]
        })
        .collect()
}
...
fn western_brackets_touching_cjk_expose_all_inter_char_boundaries() {
    ...
    let clusters = text
        .chars()
        .enumerate()
        .map(|(index, character)| Cluster::new(
            TextRange::new(index as i32, index as i32 + 1),
            character.to_string(),
            if matches!(character, '(' | ')') { "latin".to_owned() } else { "cjk".to_owned() },
            16.0,
        ))
        .collect::<Vec<_>>();
    let roles = text
        .chars()
        .map(|character| if matches!(character, '(' | ')') { FontRole::LatinText } else { FontRole::CjkText })
        .collect::<Vec<_>>();

    assert_eq!(HashSet::from([0, 1, 3, 4]), resolve_western_bracket_cjk_inter_char_boundaries(text, &clusters, &roles));
}
...
fn western_closing_punctuation_is_forbidden_at_automatic_line_start() {
    for (name, result) in [
        ...
        assert!(line_texts(&result, "中文)文").iter().all(|line| !line.starts_with(')')), "{name}");
        let decision = result.debug.contextual_kinsoku_decisions.iter().find(|decision| decision.source_text == ")").unwrap();
...
fn western_opening_bracket_is_forbidden_at_automatic_line_end() {
    for (name, result) in [
        ...
        assert!(line_texts(&result, "ABCD(E").iter().all(|line| !line.ends_with('(')), "{name}");
        let decision = result.debug.contextual_kinsoku_decisions.iter().find(|decision| decision.source_text == "(").unwrap();
...
fn attached_ascii_point_mark_remains_latin_and_cannot_start_wrapped_line() {
    for (name, result) in [
        ...
        assert!(line_texts(&result, "中文中文,中文").iter().all(|line| !line.starts_with(',')), "{name}");
        let comma = result.clusters.iter().find(|cluster| cluster.text == ",").unwrap();
        assert_eq!("latin-primary", comma.font_key);
        assert!(result.debug.punctuation_decisions.iter().all(|decision| decision.range != comma.range));
        let decision = result.debug.contextual_kinsoku_decisions.iter().find(|decision| decision.range == comma.range).unwrap();
        assert_eq!("LineStart", decision.forbidden_position);
...
fn paired_latin_curly_quotes_keep_content_across_both_line_edges() {
    for (name, result) in [
        ...
        assert!(line_texts(&result, "“ABCD”E").iter().all(|line| !line.starts_with('”')), "{name}");
        let decision = result.debug.contextual_kinsoku_decisions.iter().find(|decision| decision.source_text == "”").unwrap();
        assert_eq!("Uax14WesternPunctuationBoundary:PairedClosingQuote", decision.reason);
    }
}
