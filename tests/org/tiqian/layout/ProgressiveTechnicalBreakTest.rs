...
fn source_whitespace_capacity_keeps_structural_tier_ahead_of_syllable() {
    ...
    let clusters = vec![
        cluster(0, "a", 20.0), cluster(1, " ", 4.0), cluster(2, "b", 28.0),
        cluster(3, "/", 28.0), cluster(4, "c", 2.0), cluster(5, "d", 20.0),
    ];
    let opportunities = HashMap::from([
        (2, ProgressiveBreakOpportunity::with_preceding_whitespace_stretch_capacity(ProgressiveBreakTier::Whitespace, span, 4.0)),
        (4, ProgressiveBreakOpportunity::new(ProgressiveBreakTier::Structural, span)),
        (5, ProgressiveBreakOpportunity::new(ProgressiveBreakTier::Syllable, span)),
    ]);
