...
fn cluster(start: i32, end: i32, text: &str, advance: f32, font_key: &str) -> Cluster {
    Cluster::new(TextRange::new(start, end), text.to_owned(), font_key.to_owned(), advance)
}

fn edges(clusters: &[Cluster]) -> Vec<tiqian::org::tiqian::core::EastAsianSpacing::EastAsianSpacingEdges> {
    clusters.iter().map(|cluster| unicode_east_asian_spacing::resolved_edges(&cluster.text, "zh-Hans")).collect()
}
...
fn western_dominant_line_does_not_stretch_around_cjk_punctuation() {
    ...
    let roles = vec![FontRole::LatinText, FontRole::CjkPunctuation, FontRole::LatinText, FontRole::CjkPunctuation, FontRole::CjkPunctuation, FontRole::LatinText];
...
fn typed_sino_western_space_is_capped_before_final_uniform_spacing() {
    let clusters = vec![
        cluster(0, 1, "中", EM, "cjk"), cluster(1, 2, " ", 0.25 * EM, "latin"),
        cluster(2, 4, "Hi", 2.0 * EM, "latin"), cluster(4, 5, "中", EM, "cjk"), cluster(5, 6, "中", EM, "cjk"),
    ];
    let roles = vec![FontRole::CjkText, FontRole::LatinText, FontRole::LatinText, FontRole::CjkText, FontRole::CjkText];
    let natural: f32 = clusters.iter().map(|cluster| cluster.advance).sum();
    let plan = justify(&clusters, &roles, natural + 2.0 * EM, |_| {});

    let sino: Vec<_> = plan.allocations.iter().filter(|allocation| allocation.kind == GlueKind::CjkLatinSpace).collect();
    assert_eq!(HashSet::from([1, 2]), sino.iter().map(|allocation| allocation.target_cluster_index).collect());
    ...
    assert_eq!(HashSet::from([1, 2, 3]), uniform.iter().map(|allocation| allocation.target_cluster_index).collect());
    assert!(uniform.iter().all(|allocation| (allocation.delta - 0.5 * EM).abs() < 0.001));
    assert_eq!(0.0, plan.unfilled_deficit);
}
...
fn inseparable_number_symbol_boundary_never_stretches() {
    let clusters = vec![
        cluster(0, 1, "中", EM, "cjk"), cluster(1, 3, "50", 2.0 * EM, "latin"),
        cluster(3, 4, "%", EM, "cjk"), cluster(4, 5, "中", EM, "cjk"), cluster(5, 6, "中", EM, "cjk"),
    ];
    let roles = vec![FontRole::CjkText, FontRole::LatinText, FontRole::CjkPunctuation, FontRole::CjkText, FontRole::CjkText];
    let natural: f32 = clusters.iter().map(|cluster| cluster.advance).sum();
    let plan = justify(&clusters, &roles, natural + EM, |request| {
        request.no_stretch_boundary_after_clusters = HashSet::from([1]);
    });

    assert!(plan.allocations.iter().all(|allocation| !(allocation.target_cluster_index == 1 && matches!(allocation.kind, GlueKind::CjkLatinSpace | GlueKind::CjkInterChar))));
    assert_eq!(0.0, plan.unfilled_deficit);
}
...
fn fixed_sino_western_gap_does_not_join_final_uniform_spacing() {
    let clusters = vec![
        cluster(0, 1, "中", EM, "cjk"), cluster(1, 3, "Hi", 2.0 * EM, "latin"),
        cluster(3, 4, "中", EM, "cjk"), cluster(4, 5, "中", EM, "cjk"),
    ];
    let roles = vec![FontRole::CjkText, FontRole::LatinText, FontRole::CjkText, FontRole::CjkText];
    let natural: f32 = clusters.iter().map(|cluster| cluster.advance).sum();
    let plan = justify(&clusters, &roles, natural + EM, |request| {
