...
fn layout_reference(text: &str) -> tiqian::org::tiqian::core::LayoutModel::LayoutResult {
    ...
    ExplainableStubParagraphLayoutEngine::default().layout(
        LayoutInput::builder(
            TiqianTextContent::builder(text.to_owned()).spans(vec![attached_span(TextRange::new(start, start + 3))]).build(),
...
fn virtual_punctuation_boundary_compresses_only_when_followed_by_punctuation() {
    ...
    let decision = compressed
        ...
        .spacing_decisions
        .iter()
        .find(|decision| decision.reason.starts_with("AttachedInlineVirtualPunctuationBoundary"))
        .unwrap();
    ...
    let decision = natural
        ...
        .find(|decision| decision.reason.starts_with("AttachedInlineVirtualPunctuationBoundary"))
        .unwrap();
    assert_eq!("AttachedInlineVirtualPunctuationBoundary:natural", decision.reason);
...
fn attached_reference_never_starts_wrapped_line_for_supported_breakers() {
    ...
    for (name, breaker) in [
        ("greedy", Box::new(GreedyLineBreaker::default()) as Box<dyn tiqian::org::tiqian::layout::LineBreaker::LineBreaker>),
        ("lookahead", Box::new(LookaheadLineBreaker::default()) as Box<dyn tiqian::org::tiqian::layout::LineBreaker::LineBreaker>),
        ("paragraph-dp", Box::new(ParagraphDpLineBreaker::default()) as Box<dyn tiqian::org::tiqian::layout::LineBreaker::LineBreaker>),
    ] {
        let mut engine = ExplainableStubParagraphLayoutEngine::default();
        engine.line_breaker = breaker;
        let result = engine.layout(
            LayoutInput::builder(
                TiqianTextContent::builder(text.to_owned()).spans(vec![attached_span(range)]).build(),
        ...
        assert!(result.lines.iter().all(|line| line.range.start() != range.start()), "{name}: {:?}", result.lines);
        assert!(result.lines.iter().any(|line| line.range.start() < range.start() && line.range.end() >= range.end()), "{name}: {:?}", result.lines);
    }
}
