...
fn technical_layout(text: &str, max_width: f32) -> tiqian::org::tiqian::core::LayoutModel::LayoutResult {
    ...
    engine.layout(
        LayoutInput::builder(
            TiqianTextContent::builder(text.to_owned())
                .line_break_spans(vec![LineBreakSpan { range, policy: LineBreakPolicy::ProgressiveTechnical }])
                .build(),
...
fn rejected_letter_digit_structural_offsets_remain_emergency_cuts() {
    ...
    let emergency: Vec<_> = result.debug.break_opportunity_decisions.iter()
...
fn hash_inside_technical_url_skips_syllable_classification() {
    ...
    let syllable_offsets: Vec<_> = result.debug.break_opportunity_decisions.iter()
        .filter(|decision| decision.tier.as_deref() == Some("Syllable"))
        .flat_map(|decision| decision.break_offsets.iter().copied())
        .collect();

    assert!(syllable_offsets.iter().all(|offset| *offset <= hash_start || *offset >= text.len() as i32), "{syllable_offsets:?}");
    assert!(result.lines.iter().all(|line| line.hyphen_advance == 0.0));
}
...
fn technical_hash_uses_emergency_tracking_to_fill_auto_wrapped_lines() {
    ...
    let auto_lines: Vec<_> = result.lines.iter().filter(|line| line.end_reason == LineEndReason::AutoWrap).collect();

    assert!(!auto_lines.is_empty());
    assert!(auto_lines.iter().all(|line| (line.visual_width - 101.0).abs() < 0.001));
    assert!(result.debug.justification_decisions.iter().flat_map(|decision| &decision.allocations).any(|allocation| {
        allocation.kind == "EmergencyGraphemeTracking"
            && allocation.reason == "TerminalTechnicalEmergencyTracking:ProgressiveTechnicalSpan"
    }));
}
...
fn long_all_caps_word_is_not_tracking_eligible() {
    ...
    assert!(result.debug.emergency_tracking_eligibility_decisions.is_empty());
    assert!(result.debug.justification_decisions.iter().flat_map(|decision| &decision.allocations)
...
fn opaque_hard_break_keeps_combining_grapheme_intact() {
    ...
    assert!(result.clusters.iter().all(|cluster| {
        cluster.range.start() != combining_mark_offset && cluster.range.end() != combining_mark_offset
    }));
}
