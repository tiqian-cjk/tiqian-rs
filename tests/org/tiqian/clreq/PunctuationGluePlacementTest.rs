...
fn mainland_anchors_closing_and_pause_stop_to_trailing() {
    ...
    assert_eq!(GlueSide::TrailingOnly, placement.glue_side_for(PunctuationClass::Closing));
    assert_eq!(GlueSide::TrailingOnly, placement.glue_side_for(PunctuationClass::PauseOrStop));
}
...
fn mainland_splits_symmetric_punctuation_on_both_sides() {
    let placement = PunctuationGluePlacement::MainlandSimplified;
    for class in [PunctuationClass::MiddleDot, PunctuationClass::Ellipsis, PunctuationClass::Dash, PunctuationClass::Quote] {
...
fn traditional_centres_opening_closing_and_pause_stop() {
    ...
    for class in [PunctuationClass::Opening, PunctuationClass::Closing, PunctuationClass::PauseOrStop] {
        assert_eq!(GlueSide::BothSides, placement.glue_side_for(class));
    }
}
...
fn region_and_built_in_profiles_use_expected_placement() {
    ...
    assert_eq!(PunctuationGluePlacement::Traditional, ClreqProfile::taiwan_horizontal().glue_placement);
    assert_eq!(PunctuationGluePlacement::Traditional, ClreqProfile::hong_kong_horizontal().glue_placement);
    assert_eq!(PunctuationGluePlacement::MainlandSimplified, ClreqProfile::mainland_horizontal().glue_placement);
}
