...
fn layout_fixture_goldens_match() {
    ...
    let golden_directory =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixture_layout/golden");
    ...
    if updating {
        ...
        for fixture in &fixtures {
            fs::write(
                golden_directory.join(format!("{}.txt", fixture.id)),
                dump_fixture(fixture),
            )
    ...
    let golden_ids = fs::read_dir(&golden_directory)
        ...
        .map(|entry| {
            entry
                .expect("read fixture golden entry")
                .file_name()
                .into_string()
                .expect("UTF-8 fixture golden name")
        })
        .map(|name| {
            name.strip_suffix(".txt")
                .expect("fixture golden must use .txt extension")
                .to_owned()
        })
        .collect::<BTreeSet<_>>();
    let expected_ids = fixture_ids
        .iter()
        .map(|id| (*id).to_owned())
        .collect::<BTreeSet<_>>();
    assert_eq!(
        expected_ids, golden_ids,
        "fixture IDs and golden file names must match"
    );
    ...
    for fixture in &fixtures {
        ...
        assert_eq!(
            ...
            actual,
            "{}",
            diff_message(fixture.id, &expected, &actual)
        );
    }
}
