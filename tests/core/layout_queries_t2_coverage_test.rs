...
fn positioned_clusters_separate_occupied_box_from_consumed_leading_glue_draw_origin() {
    ...
    assert_eq!(0, get_offset_for_position(&result, -3.0, 5.0));
}

#[test]
fn rich_text_segments_reuse_positioned_cluster_geometry_and_split_lines() {
    let result = sample_result();
    let span = RichTextSpan::with_paint(TextRange::new(1, 4), RichTextRole::Background, RichTextPaint::builder().argb(0x33FF0000).build());
    let segments = positioned_rich_text_segments(&result, &[span]);
    assert_eq!(2, segments.len());
    assert_eq!(TextRange::new(1, 3), segments[0].range);
    assert_eq!(Rect { left: 14.0, top: 0.0, right: 34.0, bottom: 20.0 }, segments[0].rect());
    assert_eq!(TextRange::new(3, 4), segments[1].range);
...
fn rich_text_decoration_keeps_punctuation_glue_inside_its_range() {
    ...
    let span = RichTextSpan::new(TextRange::new(1, 4), RichTextRole::Underline);
...
fn rich_text_decoration_does_not_trim_already_consumed_opening_glue_twice() {
    ...
    let span = RichTextSpan::new(TextRange::new(0, 1), RichTextRole::Underline);
    let decoration = trimmed_rich_text_decoration_segments(&result, &positioned_rich_text_segments(&result, &[span]));
...
fn custom_line_styles_reuse_the_renderer_underline_height() {
    let result = punctuation_glue_result(0.0);
    let span = RichTextSpan::new(TextRange::new(0, 4), RichTextRole::Underline);
...
fn line_through_bisects_the_ideographic_metric_box() {
    let result = background_geometry_result(vec![metric(TextRange::new(0, 3), 8.0, 2.0, "IdeographicEmBox")]);
    let span = RichTextSpan::new(TextRange::new(0, 3), RichTextRole::LineThrough);
    let segment = trimmed_rich_text_decoration_segments(&result, &positioned_rich_text_segments(&result, &[span])).remove(0);
...
fn rich_text_background_keeps_internal_gaps_but_trims_its_outer_layout_space() {
    ...
    let full = RichTextSpan::new(TextRange::new(0, 3), RichTextRole::Background);
    let final_character = RichTextSpan::new(TextRange::new(2, 3), RichTextRole::Background);
...
fn uniform_text_style_background_ignores_fallback_face_height_and_adds_padding() {
    let result = background_geometry_result(vec![metric(TextRange::new(0, 1), 8.0, 2.0, "IdeographicEmBox"), metric(TextRange::new(1, 3), 12.0, 4.0, "RawFontBox")]);
    ...
    let first = RichTextSpan::with_paint(TextRange::new(0, 1), RichTextRole::Background, paint.clone());
    let mixed = RichTextSpan::with_paint(TextRange::new(0, 3), RichTextRole::Background, paint);
...
fn background_continuation_corners_keep_only_true_source_ends_fully_rounded() {
    let span = Arc::new(RichTextSpan::with_paint(TextRange::new(0, 12), RichTextRole::InlineCode, RichTextPaint::builder().background(RichTextBackgroundPaint::builder().corner_radius(3.0).continuation_corner_radius(1.0).build()).build()));
    let segment = |start, end| RichTextLineSegment::new(Arc::clone(&span), 0, TextRange::new(start, end), 0.0, 0.0, 40.0, 20.0, 16.0);
...
fn adjacent_backgrounds_with_the_same_style_share_one_clearance() {
    ...
    let spans = [RichTextSpan::with_paint(TextRange::new(0, 1), RichTextRole::Background, paint.clone()), RichTextSpan::with_paint(TextRange::new(1, 3), RichTextRole::Background, paint)];
...
fn adjacent_line_decorations_with_the_same_style_share_one_clearance() {
    let result = sample_result();
    let paint = RichTextPaint::builder().adjacent_same_style_clearance(2.0).build();
    let spans = [RichTextSpan::with_paint(TextRange::new(0, 1), RichTextRole::Underline, paint.clone()), RichTextSpan::with_paint(TextRange::new(1, 3), RichTextRole::Underline, paint)];
...
fn adjacent_background_and_underline_do_not_avoid_across_styles() {
    ...
    let occupied = positioned_rich_text_segments(&result, &[
        RichTextSpan::with_paint(TextRange::new(0, 1), RichTextRole::Background, paint.clone()),
        RichTextSpan::with_paint(TextRange::new(1, 3), RichTextRole::Underline, paint),
    ]);
...
fn hit_testing_chooses_offset_from_tiqian_cluster_advances() {
    ...
    for (x, y, expected) in [(3.0, 5.0, 0), (18.0, 5.0, 1), (24.0, 5.0, 2), (4.0, 25.0, 3), (30.0, 25.0, 4)] {
        assert_eq!(expected, get_offset_for_position(&result, x, y));
    }
}
...
fn external_selection_offsets_respect_directional_boundary_bias() {
    ...
    assert_eq!(4, coerce_selection_offset(&result, 3, SourceBoundaryBias::Nearest));
    assert_eq!(4, coerce_selection_offset(&result, 6, SourceBoundaryBias::Backward));
    assert_eq!(9, coerce_selection_offset(&result, 6, SourceBoundaryBias::Forward));
}
...
fn supported_source_sequence_remains_atomic_across_engine_cluster_boundaries() {
    let result = super::layout_queries_test_support::result(
        "é",
        vec![
            super::layout_queries_test_support::cluster(TextRange::new(0, 1), "e", 10.0),
            super::layout_queries_test_support::cluster(TextRange::new(1, 2), "́", 10.0),
        ],
        vec![super::layout_queries_test_support::line(TextRange::new(0, 2), tiqian::core::int_range::IntRange::new(0, 1), 15.0, 0.0, 20.0, 20.0)],
        Vec::new(), Vec::new(), tiqian::core::layout_model::LayoutDebugInfo::default(),
    );
    assert_eq!(0, coerce_selection_offset(&result, 1, SourceBoundaryBias::Backward));
    assert_eq!(2, coerce_selection_offset(&result, 1, SourceBoundaryBias::Forward));
}

#[test]
fn selection_word_boundary_expands_latin_but_keeps_han_atomic() {
    let result = word_boundary_result();
    assert_eq!(TextRange::new(2, 10), get_selection_word_boundary(&result, 6));
    assert_eq!(TextRange::new(0, 1), get_selection_word_boundary(&result, 0));
    assert_eq!(TextRange::new(1, 2), get_selection_word_boundary(&result, 1));
    assert_eq!(TextRange::new(11, 12), get_selection_word_boundary(&result, 12));
    assert_eq!(Some(TextRange::new(0, 1)), get_selection_word_boundary_for_position(&result, 5.0, 10.0));
    assert_eq!(Some(TextRange::new(2, 10)), get_selection_word_boundary_for_position(&result, 60.0, 10.0));
}
