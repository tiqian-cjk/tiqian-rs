...
use tiqian::core::geometry::{Rect, scalar_offset, text_range};
use tiqian::core::layout_queries::{
    RichTextCornerRadii, RichTextLineSegment, coerce_selection_offset, get_offset_for_position,
    get_selection_word_boundary, get_selection_word_boundary_for_position, positioned_clusters,
};
...
fn positioned_clusters_separate_occupied_box_from_consumed_leading_glue_draw_origin() {
    ...
    assert_eq!(
        scalar_offset(0),
        get_offset_for_position(&result, -3.0, 5.0)
    );
}

#[test]
fn rich_text_segments_reuse_positioned_cluster_geometry_and_split_lines() {
    let result = sample_result_with_rich_text(vec![span(
        text_range(1, 4),
...
fn rich_text_decoration_keeps_punctuation_glue_inside_its_range() {
    let result =
        punctuation_glue_result_with_rich_text(0.0, vec![span(text_range(1, 4), underline())]);
...
fn rich_text_decoration_does_not_trim_already_consumed_opening_glue_twice() {
    let result =
        punctuation_glue_result_with_rich_text(5.0, vec![span(text_range(0, 1), underline())]);
    let decoration = result.rich_text_decoration_segments();
...
fn custom_line_styles_reuse_the_renderer_underline_height() {
    let result =
        punctuation_glue_result_with_rich_text(0.0, vec![span(text_range(0, 4), underline())]);
    let segment = result.rich_text_decoration_segments().remove(0);
    assert!(
        (segment.baseline + 10.0 * 0.18 - result.rich_text_decoration_line_y(&segment, 1.0)).abs()
            < 0.001
    );
}
...
fn uniform_text_style_background_ignores_fallback_face_height_and_adds_padding() {
    ...
    let result = background_geometry_result_with_rich_text(
        vec![
            metric(text_range(0, 1), 8.0, 2.0, "IdeographicEmBox"),
            metric(text_range(1, 3), 12.0, 4.0, "RawFontBox"),
        ],
        vec![first, mixed],
    );
...
fn background_continuation_corners_keep_only_true_source_ends_fully_rounded() {
    ...
    assert_eq!(
        ...
        result.rich_text_background_corner_radii(&segment(0, 4), 0.0)
    );
    assert_eq!(
        RichTextCornerRadii {
            top_left: 1.0,
        ...
        result.rich_text_background_corner_radii(&segment(4, 8), 0.0)
    );
    assert_eq!(
        RichTextCornerRadii {
            top_left: 1.0,
        ...
        result.rich_text_background_corner_radii(&segment(8, 12), 0.0)
    );
    assert_eq!(
        RichTextCornerRadii {
            top_left: 3.0,
        ...
        result.rich_text_background_corner_radii(&segment(0, 12), 0.0)
    );
}
...
fn adjacent_backgrounds_with_the_same_style_share_one_clearance() {
    ...
    let spans = [
        ...
        span(text_range(1, 3), background(paint)),
    ];
...
fn adjacent_line_decorations_with_the_same_style_share_one_clearance() {
    let line = RichTextLinePaint {
        adjacent_same_style_clearance: 2.0,
        ..RichTextLinePaint::default()
    };
    let spans = [
        span(
            text_range(0, 1),
            RichTextLayerKind::Underline { line: line.clone() },
        ),
        span(text_range(1, 3), RichTextLayerKind::Underline { line }),
    ];
    let result = sample_result_with_rich_text(spans.into());
...
fn hit_testing_chooses_offset_from_tiqian_cluster_advances() {
    ...
    for (x, y, expected) in [
        ...
        assert_eq!(
            scalar_offset(expected),
            get_offset_for_position(&result, x, y)
        );
    }
}
...
fn external_selection_offsets_respect_directional_boundary_bias() {
    ...
    assert_eq!(
        scalar_offset(3),
        coerce_selection_offset(&result, scalar_offset(2), SourceBoundaryBias::Nearest)
    );
    assert_eq!(
        scalar_offset(3),
        coerce_selection_offset(&result, scalar_offset(4), SourceBoundaryBias::Backward)
    );
    assert_eq!(
        scalar_offset(6),
        coerce_selection_offset(&result, scalar_offset(4), SourceBoundaryBias::Forward)
    );
}
...
fn supported_source_sequence_remains_atomic_across_engine_cluster_boundaries() {
    let result = super::layout_queries_test_support::result(
        ...
        vec![super::layout_queries_test_support::line(
            text_range(0, 2),
            tiqian::core::int_range::IntRange::new(0, 1),
            15.0,
            0.0,
            20.0,
            20.0,
        ...
        Vec::new(),
        Vec::new(),
        tiqian::core::layout_model::LayoutDebugInfo::default(),
    );
    assert_eq!(
        scalar_offset(0),
        coerce_selection_offset(&result, scalar_offset(1), SourceBoundaryBias::Backward)
    );
    assert_eq!(
        scalar_offset(2),
        coerce_selection_offset(&result, scalar_offset(1), SourceBoundaryBias::Forward)
    );
}
...
fn selection_word_boundary_expands_latin_but_keeps_han_atomic() {
    ...
    assert_eq!(
        text_range(2, 10),
        get_selection_word_boundary(&result, scalar_offset(6))
    );
    assert_eq!(
        text_range(0, 1),
        get_selection_word_boundary(&result, scalar_offset(0))
    );
    assert_eq!(
        text_range(1, 2),
        get_selection_word_boundary(&result, scalar_offset(1))
    );
    assert_eq!(
        text_range(11, 12),
        get_selection_word_boundary(&result, scalar_offset(12))
    );
    assert_eq!(
        Some(text_range(0, 1)),
        get_selection_word_boundary_for_position(&result, 5.0, 10.0)
    );
    assert_eq!(
        Some(text_range(2, 10)),
        get_selection_word_boundary_for_position(&result, 60.0, 10.0)
    );
}
...
fn ruby_selection_geometry_redistributes_avoidance_spread_without_overlap() {
    ...
    assert!(
        positioned
            .windows(2)
            .all(|items| items[0].right <= items[1].left)
    );
}
