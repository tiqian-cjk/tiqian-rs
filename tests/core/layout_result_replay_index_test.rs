...
use super::layout_queries_test_support::{
    cluster, line, result, result_with_rich_text, word_boundary_result,
};
...
fn replay_index_keeps_selection_and_rich_text_on_engine_geometry() {
    ...
    assert_eq!(
        ...
        index.positioned_clusters
    );
    assert_eq!(
        result.positioned_rich_text_segments(),
        index.rich_text_segments
    );
    assert_eq!(
        result.rich_text_background_segments(),
        index.rich_text_background_segments
    );
    assert_eq!(
        result.rich_text_decoration_segments(),
        index.rich_text_decoration_segments
    );
...
fn replay_index_handles_empty_layout_and_word_selection_boundaries() {
    ...
    assert_eq!(
        scalar_offset(0),
        selection_offset_for_position(&empty_index, &empty, 20.0, 20.0)
    );
    assert_eq!(
        None,
        selection_word_range_for_position(&empty_index, &empty, 20.0, 20.0)
    );
    assert_eq!(
        Rect {
            left: 0.0,
        ...
        cursor_rect(&empty_index, &empty, scalar_offset(5))
    );
    ...
    assert_eq!(
        ...
        selection_word_range_for_position(&word_index, &words, 45.0, 10.0)
    );
}
...
fn replay_index_builds_glyph_feature_and_font_role_evidence_per_cluster() {
    ...
    assert_eq!(
        vec!["kern=1"],
        index.open_type_features_by_cluster_range[&text_range(1, 2)]
    );
    assert_eq!(
        Vec::<String>::new(),
        index.open_type_features_by_cluster_range[&text_range(2, 3)]
    );
    assert_eq!(
        Some("CjkText".to_owned()),
        index.font_role_by_cluster_range[&text_range(0, 1)]
    );
    ...
    assert_eq!(
        scalar_offset(0),
        selection_offset_for_position(&index, &result, -1.0, 10.0)
    );
    assert_eq!(
        scalar_offset(3),
        selection_offset_for_position(&index, &result, 50.0, 30.0)
    );
}
...
fn replay_index_uses_line_indent_for_empty_lines_and_nearest_cluster_for_gaps() {
    ...
    assert_eq!(
        scalar_offset(0),
        selection_offset_for_position(&empty_index, &empty_line, 5.0, 10.0)
    );
    assert_eq!(
        None,
        selection_word_range_for_position(&empty_index, &empty_line, 5.0, 10.0)
    );
    assert_eq!(
        0.0,
        cursor_rect(&empty_index, &empty_line, scalar_offset(0)).left
    );
    ...
    assert_eq!(
        scalar_offset(1),
        selection_offset_for_position(&index, &gapped, 10.0, 10.0)
    );
    assert_eq!(
        Rect {
            left: 10.0,
        ...
        cursor_rect(&index, &gapped, scalar_offset(1))
    );
}
...
fn replay_index_selects_the_nearest_source_boundary_inside_visible_cluster_gaps() {
    ...
    let first =
        PositionedCluster::builder(0, 0, text_range(0, 1), 0.0, 0.0, 10.0, 20.0, 15.0).build();
    let second =
        PositionedCluster::builder(0, 1, text_range(2, 3), 20.0, 0.0, 30.0, 20.0, 15.0).build();
    index.positioned_clusters = vec![first.clone(), second.clone()];
    index.positioned_clusters_by_line = vec![vec![first, second]];

    assert_eq!(
        scalar_offset(1),
        selection_offset_for_position(&index, &result, 12.0, 10.0)
    );
    assert_eq!(
        scalar_offset(2),
        selection_offset_for_position(&index, &result, 18.0, 10.0)
    );
}
