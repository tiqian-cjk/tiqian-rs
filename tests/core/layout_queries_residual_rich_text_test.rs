...
use tiqian::core::geometry::{TextRange, text_range};
        ...
        vec![span(
            ...
            background(
                RichTextBackgroundPaint::builder()
                    .corner_radius(6.0)
                    .continuation_corner_radius(2.0)
                    .build(),
            ),
        )],
...
fn rich_text_segments_split_on_line_breaks_and_cluster_gaps() {
    let content = result_with_rich_text(
        "abcd",
        vec![
            cluster(text_range(0, 1), "a", 10.0),
            cluster(text_range(2, 3), "c", 10.0),
            cluster(text_range(3, 4), "d", 10.0),
        ],
        vec![
            line(text_range(0, 3), IntRange::new(0, 1), 15.0, 0.0, 20.0, 20.0),
            line(
                text_range(3, 4),
                IntRange::new(2, 2),
                35.0,
    ...
    assert_eq!(text_range(0, 1), split[0].range);
    assert_eq!(text_range(2, 3), split[1].range);
    assert_eq!(text_range(3, 4), split[2].range);
    assert_eq!(0, split[0].line_index);
    assert_eq!(0, split[1].line_index);
    assert_eq!(1, split[2].line_index);
}
...
fn rich_text_segments_skip_zero_length_clusters_between_slices() {
    let content = result_with_rich_text(
        ...
        vec![
            cluster(text_range(0, 1), "a", 10.0),
            cluster(text_range(1, 1), "", 0.0),
            cluster(text_range(1, 2), "b", 10.0),
        ],
        vec![line(
            text_range(0, 2),
            IntRange::new(0, 2),
    ...
    assert_eq!(text_range(0, 2), segments[0].range);
    assert_eq!(0.0, segments[0].left);
    assert_eq!(20.0, segments[0].right);
}

#[test]
fn trimmed_decoration_segments_keep_only_decoration_roles() {
    let underline_content = result_with_rich_text(
        "ab",
        vec![
            cluster(text_range(0, 1), "a", 10.0),
            cluster(text_range(1, 2), "b", 10.0),
    ...
    assert_eq!(
        underline_content.positioned_rich_text_segments(),
        underline_content.rich_text_decoration_segments()
    );
...
fn background_segments_trim_glue_apply_padding_and_use_glyph_advances() {
    let glue = ClusterGeometryDecisionInfo::builder(
        text_range(0, 1),
        Text::from("，"),
        Text::from("，"),
        10.0,
    ...
    let content = result_with_rich_text(
        ...
        vec![GlyphRun::new(
            ...
            vec![Glyph::builder(9, text_range(1, 2), 9.0).x(1.0).build()],
            10.0,
        )],
        Vec::new(),
        vec![span(
        ...
        LayoutDebugInfo::builder()
            .geometry_decisions(vec![glue])
            .build(),
    );
    let full = content.rich_text_background_segments().remove(0);
    assert_eq!(3.0, full.left);
    assert_eq!(20.0, full.right);
    assert_eq!(15.0 - 10.0 * 0.88, full.top);
    assert_eq!(15.0 + 10.0 * 0.12, full.bottom);
}
...
fn marked_faces_use_metric_decisions_when_they_cover_the_cluster() {
    let content = result_with_rich_text(
        ...
        LayoutDebugInfo::builder()
            .metric_decisions(vec![metric(text_range(0, 2), 7.0, 3.0, "IdeographicEmBox")])
            .build(),
    );
    let box_ = content.rich_text_background_segments().remove(0);
    assert_eq!(8.0, box_.top);
    assert_eq!(18.0, box_.bottom);
}
...
fn uniform_text_style_falls_back_when_every_metric_field_differs() {
    ...
    for style in [
        TextStyle::builder()
            .font_families(vec!["other".to_owned()])
            .font_size(10.0)
            .build(),
        TextStyle::builder().font_size(11.0).build(),
        TextStyle::builder()
            .font_size(10.0)
        ...
        TextStyle::builder()
            ...
            .font_weight(700)
            .build(),
        TextStyle::builder().font_size(10.0).italic(true).build(),
        TextStyle::builder()
            .font_size(10.0)
            .baseline_shift(2.0)
            .build(),
    ] {
        let paint = RichTextBackgroundPaint::builder()
        ...
        let content = result_with_rich_text(
            ...
            vec![span(text_range(0, 2), background(paint))],
            LayoutDebugInfo::builder()
                .metric_decisions(vec![metric(text_range(1, 2), 9.0, 1.0, "LatinBox")])
                .build(),
        );
        let box_ = content.rich_text_background_segments().remove(0);
...
fn uniform_text_style_prefers_ideographic_metrics_then_any_matching_face() {
    let paint = RichTextBackgroundPaint::builder()
        .metric_policy(RichTextBackgroundMetricPolicy::UniformTextStyle)
        .build();
    let latin = result_with_rich_text(
        "ab",
        vec![
            cluster(text_range(0, 1), "a", 10.0),
            cluster(text_range(1, 2), "b", 10.0),
        ...
        Vec::new(),
        Vec::new(),
        vec![span(text_range(0, 2), background(paint.clone()))],
        LayoutDebugInfo::builder()
            .metric_decisions(vec![metric(text_range(0, 2), 9.0, 1.0, "LatinBox")])
            .build(),
    );
    let latin_box = latin.rich_text_background_segments().remove(0);
    assert_eq!(6.0, latin_box.top);
    assert_eq!(16.0, latin_box.bottom);
    let both = result_with_rich_text(
        "ab",
        vec![
            cluster(text_range(0, 1), "a", 10.0),
            cluster(text_range(1, 2), "b", 10.0),
        ...
        vec![span(text_range(0, 2), background(paint))],
        LayoutDebugInfo::builder()
            .metric_decisions(vec![
                metric(text_range(0, 1), 9.0, 1.0, "LatinBox"),
                metric(text_range(0, 2), 8.0, 2.0, "IdeographicEmBox"),
            ])
            .build(),
    );
    let ideographic = both.rich_text_background_segments().remove(0);
    assert_eq!(7.0, ideographic.top);
    assert_eq!(17.0, ideographic.bottom);
}

#[test]
fn adjacent_same_style_segments_share_clearance() {
    let paint = RichTextBackgroundPaint::builder()
        .adjacent_same_style_clearance(4.0)
        .build();
    let content = result_with_rich_text(
        "ab",
        vec![
        ...
        Vec::new(),
        vec![
            span(text_range(0, 1), background(paint.clone())),
            span(text_range(1, 2), background(paint)),
        ],
        LayoutDebugInfo::default(),
    );
    let out = content.rich_text_background_segments();
    assert_eq!(8.0, out[0].right);
    assert_eq!(12.0, out[1].left);
}
...
fn background_trailing_edge_uses_glyph_advances_when_available() {
    let content = result_with_rich_text(
        ...
        vec![GlyphRun::new(
            text_range(1, 2),
            FontFaceId::with_resource_id("test"),
            vec![Glyph::builder(2, text_range(1, 2), 5.0).build()],
            10.0,
        )],
        Vec::new(),
        vec![span(
    ...
    assert_eq!(
        15.0,
        content.rich_text_background_segments().remove(0).right
    );
}

#[test]
fn clearance_needs_same_role_and_uses_the_smaller_side() {
    let weak = RichTextBackgroundPaint::builder()
        .adjacent_same_style_clearance(2.0)
        .build();
    let strong = RichTextBackgroundPaint::builder()
        .adjacent_same_style_clearance(6.0)
        .build();
    let content = result_with_rich_text(
        "ab",
        vec![
        ...
        Vec::new(),
        vec![
            span(text_range(0, 1), background(weak)),
            span(text_range(1, 2), background(strong)),
        ],
        LayoutDebugInfo::default(),
    );
    let out = content.rich_text_background_segments();
    assert_eq!(9.0, out[0].right);
    assert_eq!(11.0, out[1].left);
}
...
fn metric_decisions_must_fully_contain_the_cluster() {
    for range in [text_range(1, 2), text_range(0, 1)] {
        let content = result_with_rich_text(
            "ab",
            vec![cluster(text_range(0, 2), "ab", 20.0)],
            vec![line(
                text_range(0, 2),
                IntRange::new(0, 0),
                15.0,
            ...
            LayoutDebugInfo::builder()
                .metric_decisions(vec![metric(range, 7.0, 3.0, "IdeographicEmBox")])
                .build(),
        );
...
fn decoration_style_resolves_inside_spans_and_at_their_edges() {
    let content = result_with_rich_text(
        ...
        vec![
            span(text_range(1, 2), underline(RichTextLinePaint::default())),
            span(text_range(2, 3), underline(RichTextLinePaint::default())),
        ],
        LayoutDebugInfo::default(),
    );
    let segments = content.positioned_rich_text_segments();
...
fn glue_trim_skips_interior_segment_edges() {
    ...
    .build();
    let right_content = result_with_rich_text(
        "ab",
        vec![cluster(text_range(0, 2), "ab", 20.0)],
        vec![line(
            text_range(0, 2),
        ...
        LayoutDebugInfo::builder()
            .geometry_decisions(vec![glue.clone()])
            .build(),
    );
    assert_eq!(
        10.0,
        right_content.rich_text_background_segments().remove(0).left
    );
    let left_content = result_with_rich_text(
        "ab",
        vec![cluster(text_range(0, 2), "ab", 20.0)],
        vec![line(
            text_range(0, 2),
    ...
    assert_eq!(
        10.0,
        left_content.rich_text_background_segments().remove(0).right
    );
}
...
fn background_segment_outside_every_span_uses_the_paragraph_style() {
    let content = result_with_rich_text(
        ...
        vec![TextSpan {
            ...
            style: TextStyle::builder().font_size(40.0).build(),
        }],
        vec![
            span(
                text_range(0, 1),
...
fn background_trailing_edge_picks_the_largest_glyph_advance() {
    let content = result_with_rich_text(
        ...
        vec![GlyphRun::new(
            ...
            FontFaceId::with_resource_id("test"),
            vec![
                Glyph::builder(1, text_range(1, 2), 5.0).build(),
                Glyph::builder(2, text_range(1, 2), 6.0).build(),
            ],
            10.0,
        )],
        Vec::new(),
...
fn background_trailing_edge_keeps_the_first_glyph_when_it_is_largest() {
    let content = result_with_rich_text(
        ...
        vec![GlyphRun::new(
            ...
            FontFaceId::with_resource_id("test"),
            vec![
                Glyph::builder(1, text_range(1, 2), 6.0).build(),
                Glyph::builder(2, text_range(1, 2), 5.0).build(),
            ],
            10.0,
        )],
        Vec::new(),
...
fn uniform_text_style_policy_picks_the_last_matching_span() {
    ...
    let content = result_with_rich_text(
        ...
        vec![span(text_range(2, 3), background(paint))],
        LayoutDebugInfo::default(),
    );
...
fn uniform_text_style_policy_keeps_the_earlier_span_when_a_later_one_misses() {
    ...
    let content = result_with_rich_text(
        ...
        vec![span(text_range(0, 1), background(paint))],
        LayoutDebugInfo::default(),
    );
    assert_eq!(0.0, content.rich_text_background_segments().remove(0).top);
}
