...
fn segment(range: TextRange, layer: RichTextLayer, span_range: TextRange, left: f32, right: f32) -> RichTextLineSegment {
    RichTextLineSegment::new(Arc::new(RichTextSpan { range: span_range, layers: vec![layer], semantics: Vec::new() }), 0, range, left, 0.0, right, 20.0, 15.0)
}
...
fn resolved_corner_radii_rejects_invalid_insets_and_resolves_continuations() {
    let content = segment(text_range(1, 2), background(RichTextBackgroundPaint::builder().corner_radius(6.0).continuation_corner_radius(2.0).build()), text_range(0, 3), 0.0, 30.0);
...
fn rich_text_segments_split_on_line_breaks_and_cluster_gaps() {
    ...
    let split = positioned_rich_text_segments(&content, &[span(text_range(0, 4), underline(RichTextLinePaint::default()))]);
...
fn rich_text_segments_skip_zero_length_clusters_between_slices() {
    ...
    let segments = positioned_rich_text_segments(&content, &[span(text_range(0, 2), underline(RichTextLinePaint::default()))]);
...
fn trimmed_decoration_segments_keep_only_decoration_roles() {
    ...
    let underline_segment = segment(text_range(0, 2), underline(RichTextLinePaint::default()), text_range(0, 2), 0.0, 20.0);
    assert_eq!(vec![underline_segment.clone()], trimmed_rich_text_decoration_segments(&content, &[underline_segment]));
    let background_segment = segment(text_range(0, 2), background(RichTextBackgroundPaint::default()), text_range(0, 2), 0.0, 20.0);
    assert!(trimmed_rich_text_decoration_segments(&content, &[background_segment]).is_empty());
}

#[test]
fn background_segments_pass_through_unmatchable_segments() {
    let content = two_cluster_result(LayoutDebugInfo::default());
    let far = segment(text_range(10, 12), background(RichTextBackgroundPaint::default()), text_range(10, 12), 0.0, 20.0);
    assert_eq!(vec![far.clone()], rich_text_background_segments(&content, &[far]));
    let orphan = RichTextLineSegment::new(Arc::new(span(text_range(0, 1), background(RichTextBackgroundPaint::default()))), 5, text_range(0, 1), 0.0, 0.0, 20.0, 20.0, 15.0);
...
fn background_segments_trim_glue_apply_padding_and_use_glyph_advances() {
    ...
    let full = rich_text_background_segments(&content, &[segment(text_range(0, 2), background(RichTextBackgroundPaint::default()), text_range(0, 2), 0.0, 20.0)]).remove(0);
...
fn marked_faces_use_metric_decisions_when_they_cover_the_cluster() {
    ...
    let box_ = rich_text_background_segments(&content, &[segment(text_range(0, 2), background(RichTextBackgroundPaint::default()), text_range(0, 2), 0.0, 20.0)]).remove(0);
...
fn uniform_text_style_falls_back_when_every_metric_field_differs() {
    ...
    for style in [TextStyle::builder().font_families(vec!["other".to_owned()]).font_size(10.0).build(), TextStyle::builder().font_size(11.0).build(), TextStyle::builder().font_size(10.0).locale("ja-JP".to_owned()).build(), TextStyle::builder().font_size(10.0).font_weight(700).build(), TextStyle::builder().font_size(10.0).italic(true).build(), TextStyle::builder().font_size(10.0).baseline_shift(2.0).build()] {
        ...
        let box_ = rich_text_background_segments(&content, &[segment(text_range(0, 2), background(paint), text_range(0, 2), 0.0, 20.0)]).remove(0);
...
fn uniform_text_style_prefers_ideographic_metrics_then_any_matching_face() {
    ...
    let latin_box = rich_text_background_segments(&latin, &[segment(text_range(0, 2), background(paint.clone()), text_range(0, 2), 0.0, 20.0)]).remove(0);
    ...
    let ideographic = rich_text_background_segments(&both, &[segment(text_range(0, 2), background(paint), text_range(0, 2), 0.0, 20.0)]).remove(0);
...
fn adjacent_same_style_segments_share_clearance() {
    ...
    let out = rich_text_background_segments(&content, &[segment(text_range(0, 1), background(paint.clone()), text_range(0, 1), 0.0, 10.0), segment(text_range(1, 2), background(paint), text_range(1, 2), 10.0, 20.0)]);
...
fn decoration_line_y_requires_valid_stroke_and_decoration_roles() {
    let content = two_cluster_result(LayoutDebugInfo::default());
    let underline_segment = segment(text_range(0, 1), underline(RichTextLinePaint::default()), text_range(0, 1), 0.0, 10.0);
    assert!(std::panic::catch_unwind(|| rich_text_decoration_line_y(&content, &underline_segment, -1.0)).is_err());
    assert!(std::panic::catch_unwind(|| rich_text_decoration_line_y(&content, &underline_segment, f32::NAN)).is_err());
    let background_segment = segment(text_range(0, 1), background(RichTextBackgroundPaint::default()), text_range(0, 1), 0.0, 10.0);
    assert!(std::panic::catch_unwind(|| rich_text_decoration_line_y(&content, &background_segment, 1.0)).is_err());
}

#[test]
fn same_span_slices_across_a_source_boundary_merge_into_one_segment() {
    let content = two_cluster_result(LayoutDebugInfo::default());
    let segments = positioned_rich_text_segments(&content, &[span(text_range(0, 2), background(RichTextBackgroundPaint::default()))]);
...
fn background_trailing_edge_uses_glyph_advances_when_available() {
    ...
    assert_eq!(15.0, rich_text_background_segments(&content, &[segment(text_range(0, 2), background(RichTextBackgroundPaint::default()), text_range(0, 2), 0.0, 20.0)]).remove(0).right);
}
...
fn clearance_needs_same_role_and_uses_the_smaller_side() {
    ...
    let out = rich_text_background_segments(&content, &[segment(text_range(0, 1), background(weak), text_range(0, 1), 0.0, 10.0), segment(text_range(1, 2), background(strong), text_range(1, 2), 10.0, 20.0)]);
...
fn metric_decisions_must_fully_contain_the_cluster() {
    for range in [text_range(1, 2), text_range(0, 1)] {
        ...
        let out = rich_text_background_segments(&content, &[segment(text_range(0, 2), background(RichTextBackgroundPaint::default()), text_range(0, 2), 0.0, 20.0)]).remove(0);
...
fn decoration_style_resolves_inside_spans_and_at_their_edges() {
    ...
    assert_eq!(16.8, rich_text_decoration_line_y(&content, &segment(text_range(1, 2), underline(RichTextLinePaint::default()), text_range(1, 2), 10.0, 20.0), 1.0));
    assert_eq!(18.6, rich_text_decoration_line_y(&content, &segment(text_range(2, 3), underline(RichTextLinePaint::default()), text_range(2, 3), 20.0, 30.0), 1.0));
}
...
fn glue_trim_skips_interior_segment_edges() {
    ...
    assert_eq!(10.0, rich_text_background_segments(&content, &[segment(text_range(1, 2), background(RichTextBackgroundPaint::default()), text_range(1, 2), 10.0, 20.0)]).remove(0).left);
    assert_eq!(10.0, rich_text_background_segments(&content, &[segment(text_range(0, 1), background(RichTextBackgroundPaint::default()), text_range(0, 1), 0.0, 10.0)]).remove(0).right);
}
...
fn background_segment_outside_every_span_uses_the_paragraph_style() {
    ...
    assert_eq!(6.2, rich_text_background_segments(&content, &[segment(text_range(0, 1), background(RichTextBackgroundPaint::default()), text_range(0, 1), 0.0, 10.0)]).remove(0).top);
    assert_eq!(6.2, rich_text_background_segments(&content, &[segment(text_range(2, 3), background(RichTextBackgroundPaint::default()), text_range(2, 3), 20.0, 30.0)]).remove(0).top);
}

#[test]
fn clearance_takes_the_smaller_side_whichever_segment_owns_it() {
    let content = two_cluster_result(LayoutDebugInfo::default());
    let out = rich_text_background_segments(&content, &[segment(text_range(0, 1), background(RichTextBackgroundPaint::builder().adjacent_same_style_clearance(6.0).build()), text_range(0, 1), 0.0, 10.0), segment(text_range(1, 2), background(RichTextBackgroundPaint::builder().adjacent_same_style_clearance(2.0).build()), text_range(1, 2), 10.0, 20.0)]);
    assert_eq!(9.0, out[0].right); assert_eq!(11.0, out[1].left);
}
...
fn uniform_text_style_policy_resolves_span_style_or_paragraph_style() {
    let paint = RichTextBackgroundPaint::builder().metric_policy(RichTextBackgroundMetricPolicy::UniformTextStyle).build();
    let content = result("abc", vec![cluster(text_range(0, 1), "a", 10.0), cluster(text_range(1, 2), "b", 10.0), cluster(text_range(2, 3), "c", 10.0)], vec![line(text_range(0, 3), IntRange::new(0, 2), 15.0, 0.0, 20.0, 30.0)], Vec::new(), vec![TextSpan { range: text_range(1, 2), style: TextStyle::builder().font_size(40.0).build() }], LayoutDebugInfo::default());
    assert_eq!(6.2, rich_text_background_segments(&content, &[segment(text_range(0, 1), background(paint.clone()), text_range(0, 1), 0.0, 10.0)]).remove(0).top);
    assert_eq!(0.0, rich_text_background_segments(&content, &[segment(text_range(1, 2), background(paint), text_range(1, 2), 10.0, 20.0)]).remove(0).top);
}
...
fn decoration_line_y_without_spans_uses_the_paragraph_style() {
    let content = two_cluster_result(LayoutDebugInfo::default());
    assert_eq!(16.8, rich_text_decoration_line_y(&content, &segment(text_range(0, 2), underline(RichTextLinePaint::default()), text_range(0, 2), 0.0, 20.0), 1.0));
}
...
fn background_trailing_edge_picks_the_largest_glyph_advance() {
    ...
    assert_eq!(16.0, rich_text_background_segments(&content, &[segment(text_range(0, 2), background(RichTextBackgroundPaint::default()), text_range(0, 2), 0.0, 20.0)]).remove(0).right);
}
...
fn uniform_text_style_policy_picks_the_last_matching_span() {
    ...
    assert_eq!(0.0, rich_text_background_segments(&content, &[segment(text_range(2, 3), background(paint), text_range(2, 3), 20.0, 30.0)]).remove(0).top);
}
...
fn uniform_text_style_policy_keeps_the_earlier_span_when_a_later_one_misses() {
    ...
    assert_eq!(0.0, rich_text_background_segments(&content, &[segment(text_range(0, 1), background(paint), text_range(0, 1), 0.0, 10.0)]).remove(0).top);
}
...
fn decoration_line_y_keeps_the_earlier_span_when_a_later_one_misses() {
    ...
    assert_eq!(18.6, rich_text_decoration_line_y(&content, &segment(text_range(0, 1), underline(RichTextLinePaint::default()), text_range(0, 1), 0.0, 10.0), 1.0));
}