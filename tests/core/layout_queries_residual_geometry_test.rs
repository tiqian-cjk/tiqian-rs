...
use tiqian::core::geometry::{Rect, TextRange, scalar_offset, text_range};
    ...
    let content = result(
        ...
        vec![GlyphRun::new(
            ...
            vec![
                ...
                Glyph::builder(3, text_range(5, 6), 10.0)
                    .bounds(Some(Rect {
                        left: 0.0,
                        top: 0.0,
                        right: 1.0,
...
fn empty_line_results_short_circuit_every_query() {
    ...
    assert_eq!(
        ...
        get_bounding_box(&content, scalar_offset(0))
    );
    assert_eq!(
        Rect {
            left: 0.0,
        ...
        get_cursor_rect(&content, scalar_offset(0))
    );
    assert_eq!(
        scalar_offset(0),
        get_offset_for_position(&content, 5.0, 5.0)
    );
    assert_eq!(
        scalar_offset(0),
        get_selection_offset_for_position(&content, 5.0, 5.0)
    );
...
fn bounding_box_falls_back_to_the_cursor_rect_at_cluster_gaps() {
    ...
    assert_eq!(
        ...
        get_bounding_box(&content, scalar_offset(1))
    );
    assert_eq!(
        Rect {
            left: 20.0,
        ...
        get_bounding_box(&content, scalar_offset(3))
    );
...
fn cursor_rect_covers_empty_lines_empty_clusters_and_multi_unit_clusters() {
    let empty = result(
        "a",
        Vec::new(),
        vec![line(text_range(0, 0), IntRange::new(0, -1), 15.0, 0.0, 20.0, 0.0).clone()],
        Vec::new(),
        Vec::new(),
        LayoutDebugInfo::default(),
    );
    ...
    let stops = result(
        ...
        vec![GlyphRun::new(
            ...
            FontFaceId::with_resource_id("test"),
            vec![
                Glyph::builder(1, text_range(0, 2), 10.0).x(0.0).build(),
                Glyph::builder(2, text_range(0, 2), 10.0).x(12.0).build(),
            ],
            20.0,
        )],
        Vec::new(),
...
fn offset_for_position_covers_vertical_distances_and_nan_points() {
    ...
    assert_eq!(
        scalar_offset(0),
        get_offset_for_position(&content, 2.0, -50.0)
    );
    assert_eq!(
        scalar_offset(2),
        get_offset_for_position(&content, 5.0, 90.0)
    );
    assert_eq!(
        scalar_offset(2),
        get_selection_offset_for_position(&content, 5.0, 30.0)
    );
    let stops = result(
        "ab",
        vec![cluster(text_range(0, 2), "ab", 20.0)],
        ...
        vec![GlyphRun::new(
            ...
            FontFaceId::with_resource_id("test"),
            vec![
                Glyph::builder(1, text_range(0, 2), 10.0).build(),
                Glyph::builder(2, text_range(0, 2), 10.0).x(10.0).build(),
            ],
            20.0,
        )],
        Vec::new(),
    ...
    assert_eq!(
        scalar_offset(0),
        get_offset_for_position(&stops, f32::NAN, 5.0)
    );
    assert_eq!(
        scalar_offset(0),
        get_selection_offset_for_position(&stops, f32::NAN, 5.0)
    );
}
...
fn ruby_geometry_redistributes_selection_boxes_and_drops_source_stops() {
    let debug = LayoutDebugInfo::builder()
        .ruby_decisions(vec![
            RubyDecisionInfo::builder(text_range(0, 3), Text::from("zhù"), 0, 15.0, 4.0, 6.0, 0.0)
                .width(30.0)
                .build(),
        ])
        .build();
    let content = result(
        "abc",
        vec![
            cluster(text_range(0, 2), "ab", 20.0),
            cluster(text_range(2, 3), "c", 10.0),
        ],
        vec![line(
            text_range(0, 3),
...
fn positioned_clusters_and_segments_return_empty_without_lines() {
    let no_lines = result_with_rich_text(
        "ab",
...
fn glyph_ink_bounds_skips_unusable_glyphs_and_reports_null() {
    ...
    let nan = result(
        ...
        vec![GlyphRun::new(
            ...
            FontFaceId::with_resource_id("test"),
            vec![
                Glyph::builder(9, text_range(1, 2), 9.0)
                    .x(f32::NAN)
                    .bounds(Some(Rect {
                        left: 1.0,
                        top: 2.0,
...
fn line_for_offset_inside_a_range_takes_the_zero_distance_arm() {
    let content = result(
        ...
        vec![
            line(text_range(0, 2), IntRange::new(0, 0), 15.0, 0.0, 20.0, 10.0),
            line(
                text_range(4, 5),
                IntRange::new(1, 1),
                35.0,
...
fn ruby_spread_shifts_selection_boxes_and_zero_width_rubies_are_ignored() {
    let geometry = |range, text, spread, advance| {
    ...
    let debug = LayoutDebugInfo::builder()
        .geometry_decisions(vec![
            geometry(text_range(0, 2), "ab", 5.0, 20.0),
            geometry(text_range(2, 3), "c", 2.0, 10.0),
        ])
        .ruby_decisions(vec![
            RubyDecisionInfo::builder(text_range(0, 3), Text::from("zhù"), 0, 15.0, 4.0, 6.0, 0.0)
                .width(30.0)
                .build(),
            RubyDecisionInfo::builder(text_range(2, 3), Text::from("x"), 0, 25.0, 4.0, 6.0, 0.0)
                .width(0.0)
                .build(),
        ])
        .build();
...
fn no_arg_positioned_clusters_walks_every_line() {
    let content = result(
        ...
        vec![
            ...
            line(text_range(4, 4), IntRange::new(2, 1), 55.0, 40.0, 60.0, 0.0),
        ],
        Vec::new(),
        Vec::new(),
        LayoutDebugInfo::default(),
...
fn nearest_line_search_covers_both_lambda_copies_of_each_arm() {
    let content = result(
        ...
        vec![
            cluster(text_range(2, 3), "c", 10.0),
            cluster(text_range(3, 4), "d", 10.0),
            cluster(text_range(6, 7), "g", 10.0),
            cluster(text_range(7, 8), "h", 10.0),
        ],
        vec![
            line(text_range(2, 4), IntRange::new(0, 1), 15.0, 0.0, 20.0, 20.0),
            line(
                text_range(6, 8),
                IntRange::new(2, 3),
                35.0,
