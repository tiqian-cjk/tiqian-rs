...
fn source_grapheme_boundaries_returns_single_boundary_for_empty_text() {
    let text = Text::new();
    assert_eq!(vec![0], source_grapheme_boundaries(&text, TextRange::new(0, 0)));
}
...
fn get_selection_offset_for_position_returns_start_of_first_cluster() {
    let result = LayoutResult::new(
        ...
        vec![
            ...
            Cluster::new(TextRange::new(2, 3), Text::from("c"), "latin".to_owned(), 10.0),
        ],
        Vec::new(),
        vec![LineBox::builder(
            TextRange::new(0, 3),
    ...
    assert_eq!(0, get_selection_offset_for_position(&result, 0.0, 10.0));
    assert_eq!(1, get_selection_offset_for_position(&result, 10.0, 10.0));
    assert_eq!(2, get_selection_offset_for_position(&result, 20.0, 10.0));
}
