...
use tiqian::core::geometry::{LayoutConstraints, Size, scalar_offset, text_range};
...
fn source_grapheme_boundaries_returns_single_boundary_for_empty_text() {
    ...
    assert_eq!(
        vec![scalar_offset(0)],
        source_grapheme_boundaries(&text, text_range(0, 0))
    );
}
...
fn get_selection_offset_for_position_returns_start_of_first_cluster() {
    ...
    assert_eq!(
        scalar_offset(0),
        get_selection_offset_for_position(&result, 0.0, 10.0)
    );
    assert_eq!(
        scalar_offset(1),
        get_selection_offset_for_position(&result, 10.0, 10.0)
    );
    assert_eq!(
        scalar_offset(2),
        get_selection_offset_for_position(&result, 20.0, 10.0)
    );
}
