...
fn ledger(
    ...
    atoms: &[tiqian::layout::punctuation_model::PunctuationAtom],
...
fn union_ignores_glyphs_without_bounds() {
    ...
    let glyphs = [
        Glyph::builder(1, text_range(0, 1), 8.0)
            .bounds(Some(Rect {
                left: 0.0,
                top: 0.0,
                right: 8.0,
...
fn attached_mark_walk_stops_mid_run_at_a_gap() {
    ...
    let mark = inline_object_attached_marks(
        &contiguous,
        &[
            FontRole::Unknown,
            FontRole::LatinText,
...
fn empty_text_clusters_cannot_be_attached_marks() {
    ...
    assert!(
        inline_object_attached_marks(
            &clusters,
            &[FontRole::Unknown, FontRole::LatinText],
            KinsokuLevel::Basic,
            &tiqian::layout::kinsoku_rule::ClreqKinsokuRule::default()
        )
        .is_empty()
    );
    let attachments = [InlineObjectAttachedMark {
...
fn ascii_point_mark_kinsoku_skips_empty_text_clusters() {
    let kinsoku = |clusters: &[Cluster]| {
        attached_ascii_point_mark_kinsoku(
            clusters,
            &vec![FontRole::LatinText; clusters.len()],
            clusters,
            KinsokuLevel::Basic,
            100.0,
            100.0,
    ...
    let empty_mark = vec![
        cluster("中", 0, EM, "cjk"),
        display_cluster("", "x", 1, EM, "latin"),
    ];
    assert!(kinsoku(&empty_mark).decisions.is_empty());
    let empty_previous = vec![
        display_cluster("", "x", 0, EM, "latin"),
        cluster(",", 0, EM, "latin"),
    ];
    ...
    let empty_next = vec![
        cluster("中", 0, EM, "cjk"),
        cluster(",", 1, 8.0, "latin"),
        display_cluster("", "x", 2, EM, "latin"),
    ];
    let roles = [FontRole::CjkText, FontRole::LatinText, FontRole::LatinText];
    let result = attached_ascii_point_mark_kinsoku(
        &empty_next,
        &roles,
        &empty_next,
        KinsokuLevel::Basic,
...
fn attached_run_at_paragraph_end_emits_no_auto_space() {
    ...
    let result = apply_auto_space_policy(
        &clusters,
        &[
            edge(EastAsianSpacingValue::Wide),
            edge(EastAsianSpacingValue::Other),
        ],
        &[InlineAttachment::None, InlineAttachment::Previous],
        AutoSpacePolicy::default_policy(),
        EM,
        &HashSet::new(),
        &HashSet::new(),
    );
...
fn virtual_gap_with_empty_previous_text_has_no_narrow_character() {
    ...
    let clusters = [
        display_cluster("", "y", 0, EM, "latin"),
        cluster("r", 1, EM, "latin"),
        cluster("中", 2, EM, "cjk"),
    ];
    assert!(
        apply_auto_space_policy(
            &clusters,
            &[
                edge(EastAsianSpacingValue::Narrow),
                EastAsianSpacingEdges {
...
fn inline_box_span_with_zero_net_structural_edge_still_applies_leading() {
    ...
    let result = apply_inline_box_spans(
        &clusters,
        &[
...
fn attached_boundary_with_plain_previous_cluster_keeps_the_right_budget() {
    ...
    let result = ledger(clusters, &atoms).resolve_attached_inline_punctuation_boundaries(
        &[
            InlineAttachment::None,
            InlineAttachment::Previous,
            InlineAttachment::None,
...
fn attached_trailing_glue_widens_a_budgeted_end_cluster() {
    ...
    let result = PunctuationGeometryLedger::from(
        clusters,
        &atoms,
        &PunctuationSpacingCompressionResult::new(Vec::new()),
    )
    .resolve_attached_inline_punctuation_boundaries(
        &[
            InlineAttachment::None,
            InlineAttachment::Previous,
            InlineAttachment::None,
...
fn spacing_plan_ignores_targets_outside_the_budgets() {
    ...
    let stray = vec![
        PunctuationSpacingAdjustment {
            ...
            reduction: 8.0,
            reason: "test-stray".to_owned(),
        },
        PunctuationSpacingAdjustment {
            range: text_range(8, 9),
            reduction_target_range: text_range(8, 9),
            left_char: '中',
            right_char: '。',
            natural_inner_glue: 8.0,
            adjusted_inner_glue: 0.0,
            reduction: 8.0,
            reason: "test-past-end".to_owned(),
        },
    ];
    let capacity = PunctuationGeometryLedger::from(
        clusters,
        &atoms,
        &PunctuationSpacingCompressionResult::new(stray),
    )
    .glue_capacities();
