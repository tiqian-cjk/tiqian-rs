...
fn compressed_final_mandatory_line_uses_the_compressed_commit_branch() {
    ...
    assert_eq!(
        IntRange::new(0, 2),
        solution.lines[0].cluster_range,
        "{:?}",
        solution.lines
    );
    assert_eq!(LineEndReason::MandatoryBreak, solution.lines[0].end_reason);
    assert!(
        matches!(&solution.lines[0].repair, Some(RepairOption::PushIn { reason, .. }) if reason.starts_with("LineAdjustmentPushIn")),
        "{:?}",
        solution.lines
    );
}
...
fn tier_promotion_routes_the_repair_reason_through_the_promotion_code() {
    ...
    assert!(
        matches!(&first.repair, Some(RepairOption::PushIn { reason, .. }) if reason.starts_with("ProgressiveTechnicalTierPromotion")),
        "{:?}",
        solution.lines
    );
}
...
fn mandatory_segment_filters_the_control_boundary_from_candidates() {
    ...
    config.hard_break_after_clusters = tiqian::common::HashSet::from([2]);
    let solution =
        ParagraphDpLineBreaker::default().break_lines(&clusters, &clusters, 32.0, &config);
    ...
    assert_eq!(
        5,
        solution.lines.last().unwrap().cluster_range.last(),
        "{:?}",
        solution.lines
    );
}
...
fn narrow_windows_drop_ends_at_or_below_the_line_start() {
    ...
    assert!(
        solution
            .lines
            .iter()
            .all(|line| line.cluster_range.first() == line.cluster_range.last()),
        "{:?}",
        solution.lines
    );
}
