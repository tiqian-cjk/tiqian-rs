...
use tiqian::layout::punctuation_geometry_stage::{
    InlineObjectAttachedMark, apply_auto_space_policy, apply_inline_box_spans,
    attached_ascii_point_mark_kinsoku, inline_object_attached_kinsoku,
    inline_object_attached_marks, is_attached_ascii_point_mark_at,
    is_east_asian_spacing_boundary_at, punctuation_atoms,
};
...
fn atoms(
    cluster: &Cluster,
    glyphs: &[Glyph],
) -> Vec<tiqian::layout::punctuation_model::PunctuationAtom> {
    punctuation_atoms(
        cluster,
        EM,
...
fn unmatched_glyph_counts_record_the_ambiguous_fallback() {
    ...
    let output = atoms(
        &mark,
        &[
            glyph(1, 8.0, 0.0, None),
            glyph(2, 8.0, 0.0, None),
            glyph(3, 8.0, 0.0, None),
        ],
    );
    ...
    assert!(
        output
            .iter()
            .all(|atom| atom.ink_bounds_fallback.as_deref()
                == Some("glyph-cluster-mapping-ambiguous"))
    );
...
fn attached_marks_collapse_separator_space_before_the_mark() {
    ...
    let marks = inline_object_attached_marks(
        &clusters,
        &roles,
        KinsokuLevel::Basic,
...
fn attached_marks_reject_missing_objects_and_gapped_ranges() {
    ...
    assert!(
        inline_object_attached_marks(
            ...
            &[FontRole::LatinText, FontRole::CjkPunctuation],
            KinsokuLevel::Basic,
            &rule,
        )
        .is_empty()
...
fn inline_object_kinsoku_protects_or_hangs_attached_marks() {
    ...
    let attachments = vec![InlineObjectAttachedMark {
        object_cluster_index: 0,
        separator_cluster_indices: Vec::new(),
        mark_cluster_index: 1,
    }];
    ...
    assert!(
        inline_object_attached_kinsoku(
            &clusters,
            &attachments,
            &clusters,
            KinsokuLevel::None,
    ...
    let fits = inline_object_attached_kinsoku(
        &clusters,
        &attachments,
        &clusters,
        KinsokuLevel::Basic,
    ...
    let hangs = inline_object_attached_kinsoku(
        &clusters,
        &attachments,
        &clusters,
        KinsokuLevel::Basic,
    ...
    assert!(
        inline_object_attached_kinsoku(&pair, &attachments, &pair, KinsokuLevel::Basic, 10.0, 10.0)
    ...
    let separated_attachments = vec![InlineObjectAttachedMark {
        object_cluster_index: 0,
        separator_cluster_indices: vec![1],
        mark_cluster_index: 2,
    }];
    let result = inline_object_attached_kinsoku(
        &separated,
        &separated_attachments,
        &separated,
        KinsokuLevel::Basic,
        100.0,
        100.0,
...
fn attached_ascii_point_mark_kinsoku_protects_runs() {
    ...
    assert!(
        attached_ascii_point_mark_kinsoku(
            &clusters,
            &roles,
            &clusters,
            KinsokuLevel::None,
    ...
    let fits = attached_ascii_point_mark_kinsoku(
        &clusters,
        &roles,
        &clusters,
        KinsokuLevel::Basic,
    ...
    assert!(
        fits.decisions
            .iter()
            .all(|decision| decision.reason == "AttachedAsciiPointMarkKinsoku")
    );
    let hangs = attached_ascii_point_mark_kinsoku(
        &clusters,
        &roles,
        &clusters,
        KinsokuLevel::Basic,
    ...
    let bounded = vec![
        cluster("中", 0, EM, "cjk"),
        cluster(",", 1, 8.0, "latin"),
        cluster("a", 2, 8.0, "latin"),
    ];
    let bounded_result = attached_ascii_point_mark_kinsoku(
        &bounded,
        &roles,
        &bounded,
        KinsokuLevel::Basic,
...
fn attached_ascii_point_mark_kinsoku_rejects_detached_runs() {
    ...
    assert!(
        kinsoku(&gapped, &[FontRole::CjkText, FontRole::LatinText])
            .decisions
            .is_empty()
    );
    let object_base = vec![inline_object(0), cluster(",", 1, EM, "latin")];
    assert!(
        kinsoku(&object_base, &[FontRole::Unknown, FontRole::LatinText])
            .decisions
            .is_empty()
    );
    let plain = vec![cluster("中", 0, EM, "cjk"), cluster("a", 1, EM, "latin")];
    assert!(
        kinsoku(&plain, &[FontRole::CjkText, FontRole::LatinText])
            .decisions
            .is_empty()
    );
    let cjk = vec![cluster("中", 0, EM, "cjk"), cluster("，", 1, EM, "cjk")];
    assert!(
        kinsoku(&cjk, &[FontRole::CjkText, FontRole::CjkPunctuation])
            .decisions
            .is_empty()
    );
}
...
fn typed_space_between_wide_and_narrow_is_replaced_by_the_gap() {
    ...
    let result = apply_auto_space_policy(
        &clusters,
        &[
            edges(EastAsianSpacingValue::Wide, EastAsianSpacingValue::Wide),
            edges(EastAsianSpacingValue::Other, EastAsianSpacingValue::Other),
            edges(EastAsianSpacingValue::Narrow, EastAsianSpacingValue::Narrow),
        ],
    ...
    assert_eq!(
        "TextAutoSpaceReplace:east-asian-spacing-W-space-N",
        decision.reason
    );
...
fn space_replacement_skips_disabled_mode_null_boundaries_and_exact_widths() {
    ...
    let disabled = apply_auto_space_policy(
        &clusters,
        &spacing,
        &[InlineAttachment::None; 3],
        AutoSpacePolicy::disabled(),
        EM,
        &HashSet::new(),
        &HashSet::new(),
    );
    ...
    let missing_edges = apply_auto_space_policy(
        &clusters,
        &[edges(
    ...
    let missing_attachments = apply_auto_space_policy(
        &clusters,
        &spacing,
        &[InlineAttachment::None],
        replace,
    ...
    assert!(
        apply_auto_space_policy(&[], &[], &[], replace, EM, &HashSet::new(), &HashSet::new())
            .clusters
            .is_empty()
    );
}
...
fn wide_to_narrow_boundaries_insert_leading_and_trailing_gaps() {
    ...
    let leading_result = apply_auto_space_policy(
        &leading,
        &[
            edges(EastAsianSpacingValue::Wide, EastAsianSpacingValue::Wide),
            edges(EastAsianSpacingValue::Narrow, EastAsianSpacingValue::Narrow),
        ],
        &[InlineAttachment::None; 2],
        insert,
        EM,
        &HashSet::new(),
        &HashSet::new(),
    ...
    assert_eq!(
        ...
        leading_result.decisions[0].boundary_role
    );
    assert_eq!(
        "TextAutoSpaceInsert:east-asian-spacing-W-N",
        leading_result.decisions[0].reason
    );
    ...
    let trailing_result = apply_auto_space_policy(
        &trailing,
        &[
            edges(EastAsianSpacingValue::Narrow, EastAsianSpacingValue::Narrow),
            edges(EastAsianSpacingValue::Wide, EastAsianSpacingValue::Wide),
...
fn narrow_inline_boxes_own_their_outer_auto_space() {
    ...
    assert_eq!(
        "InlineBox.Narrow",
        trailing_result.decisions[0].boundary_role
    );
    assert_eq!(
        "InlineBoxOuterAutoSpace:trailing-N-W",
        trailing_result.decisions[0].reason
    );
}
...
fn attached_runs_own_one_virtual_gap_at_their_trailing_edge() {
    let clusters = vec![
        ...
        cluster("a", 4, 8.0, "latin"),
    ];
    let result = apply_auto_space_policy(
        &clusters,
        &[
            edges(EastAsianSpacingValue::Wide, EastAsianSpacingValue::Wide),
            edges(EastAsianSpacingValue::Other, EastAsianSpacingValue::Wide),
            edges(EastAsianSpacingValue::Narrow, EastAsianSpacingValue::Narrow),
        ],
        &[
            InlineAttachment::None,
...
fn virtual_gaps_respect_narrow_to_wide_edges_and_their_neighbours() {
    ...
    let reversed = vec![
        cluster("a", 0, 8.0, "latin"),
        cluster("ref", 1, EM, "latin"),
        cluster("中", 4, EM, "cjk"),
    ];
    let reversed_result = apply_auto_space_policy(
        &reversed,
        &[
            edges(EastAsianSpacingValue::Narrow, EastAsianSpacingValue::Narrow),
            edges(EastAsianSpacingValue::Narrow, EastAsianSpacingValue::Other),
            edges(EastAsianSpacingValue::Wide, EastAsianSpacingValue::Wide),
        ],
        &attachments,
        AutoSpacePolicy::default(),
    ...
    assert_eq!(
        "AttachedInlineVirtualAutoSpace:east-asian-spacing-W-N",
        reversed_result.decisions[0].reason
    );
    let space_after = vec![
        cluster("中", 0, EM, "cjk"),
        cluster("ref", 1, EM, "latin"),
...
fn spacing_boundaries_count_each_wide_narrow_gap_once() {
    ...
    assert!(is_east_asian_spacing_boundary_at(
        1,
        &wn,
        &[
            edges(EastAsianSpacingValue::Wide, EastAsianSpacingValue::Wide),
...
fn attached_ascii_point_marks_need_a_contiguous_non_space_base() {
    assert!(is_attached_ascii_point_mark_at(
        &[cluster("中", 0, EM, "cjk"), cluster(",", 1, EM, "latin")],
        1
    ));
    assert!(!is_attached_ascii_point_mark_at(
        &[cluster("中", 0, EM, "cjk"), cluster(",", 1, EM, "latin")],
    ...
    assert!(!is_attached_ascii_point_mark_at(
        &[cluster("中", 0, EM, "cjk"), cluster("a", 1, EM, "latin")],
        1
    ));
    assert!(!is_attached_ascii_point_mark_at(
        &[
    ...
    assert!(!is_attached_ascii_point_mark_at(
        &[cluster("中", 0, EM, "cjk"), cluster(",", 2, EM, "latin")],
        1
    ));
}
...
fn inline_box_spans_add_structural_edges_and_skip_degenerate_ranges() {
    let clusters = vec![
        cluster("a", 0, 8.0, "latin"),
        cluster("b", 1, 8.0, "latin"),
        cluster("c", 2, 8.0, "latin"),
    ];
    assert_eq!(clusters, apply_inline_box_spans(&clusters, &[]).clusters);
    assert!(
        apply_inline_box_spans(
            &[],
    ...
    let skipped = apply_inline_box_spans(
        &clusters,
        &[
            InlineBoxSpan::with_edges(text_range(2, 2), 4.0, 0.0),
            InlineBoxSpan::with_edges(text_range(10, 11), 4.0, 0.0),
        ],
    );
    assert!(skipped.decisions.is_empty());
    assert!(skipped.advance_by_cluster.is_empty());
    let applied = apply_inline_box_spans(
        &clusters,
        &[
            InlineBoxSpan::with_edges(text_range(0, 1), 2.0, 0.0),
    ...
    let clamped = apply_inline_box_spans(
        &[cluster("a", 0, 2.0, "latin")],
        &[InlineBoxSpan::with_edges(text_range(0, 1), 0.0, -6.0)],
    );
