...
fn preserves_source_text_when_using_clreq_recommended_display_glyphs() {
    ...
    for (source, display) in [("……", "⋯⋯"), ("——", "⸺"), ("・", "·"), ("／", "／")]
    {
        let cluster = result
            .clusters
            .iter()
            .find(|cluster| cluster.text == source)
            .unwrap();
        ...
        |input: &FontBackendRequest, mut result: FontBackendShapingResult| {
            ...
            let clusters: Vec<_> = (0..input.range.length())
                .map(|local_offset| {
                    ...
                    Cluster::with_display_text(
                        ...
                        input
                            .display_text
                            .slice_text(text_range(local_offset, local_offset + 1)),
                        face.clone(),
                        16.0,
                    )
                })
            ...
            let glyph_runs = clusters
                ...
                .map(|(glyph_id, cluster)| {
                    let features = (cluster.text == "’")
                        .then(|| vec!["pwid".to_owned(), "palt".to_owned()])
                        .unwrap_or_default();
                    GlyphRun::with_open_type_features(
                        ...
                        cluster
                            .font_face
                            .clone()
                            .expect("fixture cluster must have a final font face"),
                        vec![
                            Glyph::builder(glyph_id as u32, cluster.range, cluster.advance)
...
fn preserves_open_type_features_as_final_glyph_run_boundaries() {
    let mut engine =
        ParagraphLayoutEngineBuilder::new(Box::new(feature_boundary_backend())).build();
    ...
    assert_eq!(
        vec![
            Vec::<String>::new(),
            vec!["pwid".to_owned(), "palt".to_owned()],
            Vec::new()
        ],
        result
            .glyph_runs
    ...
    stub_backend_with_transform(
        |input: &FontBackendRequest, mut result: FontBackendShapingResult| {
            ...
            result.shaping.glyph_runs = vec![GlyphRun::new(
                ...
                face.clone(),
                vec![
                    Glyph::builder(0, input.range, 16.0)
...
fn substitution_rolls_back_to_source_text_when_font_lacks_the_glyph() {
    ...
    assert!(
        decision
            .substitution_reason
            .ends_with("SubstitutionRollbackOnMissingGlyph")
    );
}
        ...
        |input: &FontBackendRequest, mut result: FontBackendShapingResult| {
            if input.display_text.as_str().contains('⋯') {
                result.shaping.decisions = result
                    ...
                    .into_iter()
                    .map(|decision| ShapingDecisionInfo {
                        capability_issue: Some(
                            UNVERIFIED_DISPLAY_SUBSTITUTION_COVERAGE_ISSUE.to_owned(),
                        ),
                        ..decision
                    })
                    .collect();
...
fn ellipsis_substitution_rolls_back_when_coverage_cannot_be_verified() {
    let mut engine =
        ParagraphLayoutEngineBuilder::new(Box::new(unverified_coverage_backend())).build();
    let result = engine.layout(input("中……文"));
    assert_eq!(
        "……",
        result
    ...
    stub_backend_with_transform(
        |input: &FontBackendRequest, mut result: FontBackendShapingResult| {
            ...
            result.shaping.glyph_runs = vec![GlyphRun::new(
                ...
                face.clone(),
                vec![
                    Glyph::builder(0, input.range, 32.0)
                        .render_font_face(Some(face))
                        .bounds(Some(Rect {
                            left: 2.0,
                            top: -10.0,
...
fn ambiguous_glyph_cluster_mapping_falls_back_to_policy_with_recorded_reason() {
    let mut engine =
        ParagraphLayoutEngineBuilder::new(Box::new(ambiguous_cluster_backend())).build();
    let result = engine.layout(input("……"));
...
fn multi_character_punctuation_uses_character_local_ink_bounds() {
    let mut engine =
        ParagraphLayoutEngineBuilder::new(Box::new(character_local_ink_backend())).build();
    let result = engine.layout(input("……"));
    assert_eq!(2, result.debug.punctuation_decisions.len());
    assert_eq!(
...
fn rolled_back_dash_still_keeps_its_boundaries_closed_under_justification() {
    ...
    assert_eq!("——", dash.display_text);
    assert!(
        decision.allocations.iter().all(|allocation| {
