...
fn attached_inline_virtual_sino_western_boundary_out_of_bounds() {
    ...
    let plan = justify(
        ...
        60.0,
        |request| {
            request.attached_inline_virtual_boundary_after_clusters =
                HashMap::from([(-1, -2), (2, 1), (5, 4)]);
...
fn preferred_inline_object_boundary_out_of_bounds() {
    ...
    let relation =
        InlineObjectPreferredStretch::new(InlineObjectPreferredStretchKind::Relation, 0.0, 4.0);
    let plan = justify(
        &clusters,
        &roles,
        &spacing,
        ...
        |request| {
            request.preferred_inline_object_boundary_after_clusters =
                HashMap::from([(-1, relation), (2, relation), (5, relation)]);
        },
    );
...
fn virtual_sino_western_gap_when_allow_sino_western_gap_stretch_is_false() {
    ...
    assert!(
        plan.allocations
            .iter()
            .all(|allocation| allocation.target_cluster_index != 3)
    );
}
...
fn virtual_non_sino_western_boundary_when_allow_sino_western_gap_stretch_is_false() {
    ...
    assert!(plan.allocations.iter().any(|allocation| {
        allocation.target_cluster_index == 3
            && allocation.reason == "AttachedInlineVirtualInterChar"
    }));
}
