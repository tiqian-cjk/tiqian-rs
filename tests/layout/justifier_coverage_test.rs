...
fn latin_space_latin(
    space_advance: f32,
    a_advance: f32,
    b_advance: f32,
) -> (Vec<Cluster>, Vec<FontRole>, Vec<EastAsianSpacingEdges>) {
    (
        vec![
            c("a", 0, a_advance, "lat"),
            c(" ", 1, space_advance, "lat"),
            c("b", 2, b_advance, "lat"),
        ],
        vec![
            FontRole::LatinText,
...
fn technical_whitespace_stretch_fills_and_stops_the_tier_chain() {
    ...
    let plan = justify(
        ...
        &justifier,
        |request| {
            request.technical_boundary_after_clusters =
                HashMap::from([(1, ProgressiveBreakTier::Whitespace)]);
        },
    );
...
fn technical_whitespace_requires_the_whitespace_tier_and_a_source_space() {
    ...
    let wrong_tier = justify(
        ...
        |request| {
            request.technical_boundary_after_clusters =
                HashMap::from([(1, ProgressiveBreakTier::Structural)]);
        },
    );
    assert_eq!(
        GlueKind::WordSpace,
    ...
    let wrong_cluster = justify(
        ...
        |request| {
            request.technical_boundary_after_clusters =
                HashMap::from([(0, ProgressiveBreakTier::Whitespace)]);
        },
    );
    assert_eq!(
        GlueKind::WordSpace,
...
fn zero_technical_stretch_capacity_produces_no_opportunity() {
    ...
    assert_eq!(
        ...
        plan.allocations
            .first()
            .expect("expected exactly one allocation")
            .kind
    );
...
fn word_space_at_the_cap_or_collapsed_is_skipped() {
    ...
    assert_eq!(
        ...
        at_cap_plan.fallback_reason
    );

    let collapsed = latin_space_latin(0.0, EM, EM);
    let collapsed_plan = justify(
        &collapsed.0,
        &collapsed.1,
        &collapsed.2,
        IntRange::new(0, 2),
...
fn space_gap_protection_covers_all_four_disjuncts() {
    ...
    for (after, protected_clusters) in [
        ...
        assert!(
            plan.allocations
                .iter()
                .all(|allocation| allocation.kind != GlueKind::WordSpace)
        );
        assert_eq!(0.0, plan.unfilled_deficit);
    }
}
...
fn virtual_sino_western_gap_skips_protected_and_typed_edges() {
    ...
    let typed_right = justify(
        &typed_right_clusters,
        &typed_right_roles,
        &typed_right_edges,
        IntRange::new(0, 2),
        52.0,
...
fn attached_inline_virtual_auto_space_joins_tier_two() {
    ...
    let happy = justify(
        ...
        |request| {
            ...
            request.attached_inline_virtual_sino_western_boundary_after_clusters =
                HashSet::from([2]);
        },
    );
    ...
    assert!(
        no_previous
            .allocations
            .iter()
            .all(|allocation| allocation.reason != "AttachedInlineVirtualAutoSpace")
    );

    let target_out_of_range = justify(
        &clusters,
        &roles,
        &edges,
        IntRange::new(0, 2),
...
fn typed_sino_western_space_stretches_from_its_base() {
    ...
    assert!(
        collapsed_plan
            .allocations
            .iter()
            .all(|allocation| allocation.target_cluster_index != 1 || allocation.delta <= 0.0)
    );
}
...
fn typed_sino_western_space_needs_both_edges_to_pair() {
    ...
    assert!(plan.allocations.iter().all(|allocation| !matches!(
        allocation.kind,
        GlueKind::WordSpace | GlueKind::CjkLatinSpace
    )));
    assert_eq!(0.0, plan.unfilled_deficit);
}
...
fn preferred_inline_object_stretch_runs_by_semantic_kind() {
    ...
    for (kind, reason, glue) in [
        ...
        let plan = justify(
            ...
            &Justifier::default(),
            |request| {
                request.preferred_inline_object_boundary_after_clusters =
                    HashMap::from([(1, InlineObjectPreferredStretch::new(kind, 4.0, 8.0))]);
            },
        );
        let [allocation] = plan.allocations.as_slice() else {
            panic!("expected exactly one allocation: {:?}", plan.allocations);
    ...
    assert!(
        closed
            .allocations
            .iter()
            .all(|allocation| allocation.kind != GlueKind::InlineObjectRelation)
    );
}
...
fn preferred_inline_object_kinds_chain_until_filled() {
    ...
    assert!(plan.allocations.iter().all(|allocation| matches!(
        allocation.kind,
        GlueKind::InlineObjectPunctuationTrailing | GlueKind::InlineObjectRelation
    )));
}
...
fn uniform_object_boundary_opens_the_gate_and_fills() {
    ...
    assert!(
        plan.allocations
            .iter()
            .any(|allocation| allocation.kind == GlueKind::InlineObjectBoundary)
    );
    assert_eq!(0.0, plan.unfilled_deficit);
}
...
fn emergency_tracking_fills_the_residual_for_authorized_boundaries() {
    ...
    let plan = justify(
        ...
        &Justifier::default(),
        |request| {
            request.emergency_tracking_boundary_after_clusters =
                HashMap::from([(0, "token".to_owned())]);
        },
    );
    let [allocation] = plan.allocations.as_slice() else {
        panic!("expected exactly one allocation: {:?}", plan.allocations);
    ...
    let preferred = justify(
        ...
        |request| {
            request.emergency_tracking_boundary_after_clusters =
                HashMap::from([(0, "token".to_owned())]);
            request.preferred_emergency_tracking_boundary_after_clusters =
                HashMap::from([(0, "code".to_owned())]);
        },
    );
    let [preferred_allocation] = preferred.allocations.as_slice() else {
        panic!(
            "expected exactly one preferred allocation: {:?}",
            preferred.allocations
        );
    };
    assert_eq!(
        "TerminalTechnicalEmergencyTracking:code",
        preferred_allocation.reason
    );
    assert_eq!(
        GlueKind::EmergencyGraphemeTracking,
        preferred_allocation.kind
    );
}
...
fn uniform_text_boundaries_exclude_protected_classes() {
    ...
    assert_eq!(
        ...
        plain
            .allocations
            .first()
            .expect("expected plain allocation")
            .kind
    );
    ...
    assert_eq!(
        "WesternBracketCjkInterChar",
        bracket
            .allocations
            .first()
            .expect("expected bracket allocation")
            .reason
    );
    ...
    assert_eq!(
        ...
        virtual_owned
            .allocations
            .first()
            .expect("expected virtual allocation")
            .reason
    );

    let uniform_object = justify(
        &clusters,
        &roles,
        &edges,
        IntRange::new(0, 1),
    ...
    assert_eq!(
        ...
        uniform_object
            .allocations
            .first()
            .expect("expected object allocation")
            .kind
    );

    let bracket_physical = justify(
        &clusters,
        &roles,
        &edges,
        IntRange::new(0, 1),
    ...
    assert_eq!(
        ...
        bracket_object
            .allocations
            .first()
            .expect("expected bracket object allocation")
            .kind
    );
}
...
fn attached_inline_virtual_inter_char_honours_no_stretch_protection() {
    ...
    assert_eq!(
        ...
        happy
            .allocations
            .first()
            .expect("expected attached allocation")
            .reason
    );
    ...
    for (no_stretch, no_stretch_after) in [
        ...
        assert!(
            plan.allocations
                .iter()
                .all(|allocation| allocation.reason != "AttachedInlineVirtualInterChar")
        );
        assert!(plan.unfilled_deficit > 0.0);
    }

    let promoted = justify(
        &clusters,
        &roles,
        &edges,
        IntRange::new(0, 3),
...
fn paragraph_edge_space_lines_cover_the_boundary_guards() {
    ...
    let leading_gap = leading_plan
        .allocations
        .iter()
        .find(|allocation| allocation.kind == GlueKind::CjkLatinSpace)
        .expect("expected leading Sino-Western gap");
    ...
    let trailing_gap = trailing_plan
        .allocations
        .iter()
        .find(|allocation| allocation.kind == GlueKind::CjkLatinSpace)
        .expect("expected trailing Sino-Western gap");
