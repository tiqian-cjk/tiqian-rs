...
fn style_at_and_style_deltas_in_prepared_paragraph_json() {
    ...
    .build();
    let clusters = vec![
        Cluster::new(text_range(0, 1), Text::from("甲"), FontFaceId::with_resource_id("k"), 16.0),
        Cluster::new(text_range(1, 2), Text::from("乙"), FontFaceId::with_resource_id("k"), 20.0),
        Cluster::new(text_range(2, 3), Text::from("丙"), FontFaceId::with_resource_id("k"), 16.0),
    ];
    ...
    let result = LayoutResult::new(input, Size { width: 200.0, height: 24.0 }, clusters, vec![GlyphRun::with_open_type_features(text_range(0, 3), FontFaceId::with_resource_id("k"), glyphs, 52.0, vec!["liga".into(), "dlig".into()])], vec![line(text_range(0, 3), IntRange::new(0, 2), 52.0)]);
...
fn inline_box_edges_and_emphasis_dots_filter() {
    let input = LayoutInput::builder(TiqianTextContent::new(Text::from("甲乙")), LayoutConstraints::with_defaults(200.0))
        ...
        .build();
    let clusters = vec![Cluster::new(text_range(0, 1), Text::from("甲"), FontFaceId::with_resource_id("k"), 16.0), Cluster::new(text_range(1, 2), Text::from("乙"), FontFaceId::with_resource_id("k"), 16.0)];
    ...
    let result = LayoutResult::with_debug(input, Size { width: 200.0, height: 24.0 }, clusters, vec![GlyphRun::new(text_range(0, 2), FontFaceId::with_resource_id("k"), glyphs, 32.0)], vec![line(text_range(0, 2), IntRange::new(0, 1), 32.0)], debug);
    let json = to_prepared_paragraph_json(&result, true);
...
fn dash_shaping_decision_with_glyph_ids() {
    ...
    let cluster = Cluster::new(text_range(0, 2), Text::from("——"), FontFaceId::with_resource_id("k"), 32.0);
    let decision = ShapingDecisionInfo::builder(text_range(0, 2), Text::from("——"), Text::from("——"), Some(FontFaceId::with_resource_id("k")), 2, 32.0, "test".into(), "DashRule".into()).language(Some("zh".into())).strategy(Some("DashTwoEmLigature".into())).build();
    let result = LayoutResult::with_debug(input, Size { width: 200.0, height: 24.0 }, vec![cluster], vec![GlyphRun::new(text_range(0, 2), FontFaceId::with_resource_id("k"), vec![Glyph::builder(42, text_range(0, 2), 16.0).build(), Glyph::builder(43, text_range(0, 2), 16.0).build()], 32.0)], vec![line(text_range(0, 2), IntRange::new(0, 0), 32.0)], LayoutDebugInfo::builder().shaping_decisions(vec![decision]).build());
    let json = to_prepared_paragraph_json(&result, true);
    for expected in ["\"glyphIds\":\"42,43\"", "\"shapingLanguage\":\"zh\"", "\"resolvedFace\":\"k#0@default\"", "\"naturalWidth\":32"] { assert!(json.contains(expected), "missing {expected}: {json}"); }
}
