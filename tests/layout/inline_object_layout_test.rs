...
use tiqian::core::text_model::{
    ...
    InlineObjectPreferredStretchKind, InlineObjectSpan, LayoutInput, LineLengthGrid,
    ParagraphStyle, TextStyle, TiqianTextContent,
};
...
fn inline_object_reuses_existing_interline_space_without_moving_baseline_grid() {
    ...
    assert!(
        (with_object.lines[1].baseline
            - 20.0
            - (with_object.lines[0].baseline + decision.base_face_descent)
            >= decision.minimum_clearance - 0.001)
    );
}
...
fn inline_object_expands_only_the_boundary_with_actual_collision() {
    ...
    assert!(
        (without_clearance.lines[1].baseline - without_clearance.lines[0].baseline - 30.0).abs()
            < 0.001
    );
    assert_eq!(
        0.0,
...
fn inline_object_is_one_indivisible_break_cluster() {
    ...
    let index = result
        .clusters
        .iter()
        .position(|cluster| cluster.range == text_range(1, 2))
        .unwrap() as i32;
    assert!(
        result
            .lines
            .iter()
            .any(|line| line.cluster_range.first() == index && line.cluster_range.last() == index)
    );
}
...
fn inline_object_keeps_alternate_source_text_while_skipping_its_glyph_shaping() {
    ...
    let object = result
        .clusters
        .iter()
        .find(|cluster| cluster.range == text_range(1, 3))
        .unwrap();
...
fn formula_boundary_compression_pushes_attached_comma_into_previous_line() {
    ...
    assert!(
        repair
            .push_in_allocations
            .iter()
            .any(|allocation| allocation.cluster_range == text_range(0, 2)
                && allocation.shrink > 0.0)
    );
}

#[test]
fn per_atom_formula_chain_never_breaks_mid_run() {
    let closed = InlineObjectBoundaryAdjustment::builder()
    ...
    for line in &result.lines {
        assert!(
            ![scalar_offset(2), scalar_offset(3), scalar_offset(4)].contains(&line.range.end()),
            "{:?}",
            result.lines
        );
    }
            ...
            for width in [24.0, 32.0, 36.0, 48.0, 64.0] {
                ...
                assert!(
                    result
                        ...
                        .iter()
                        .any(|decision| decision.source_text == comma.to_string()
                            && decision.reason == "InlineObjectAttachedKinsoku")
                );
            }
        }
    }
        ...
        for width in [32.0, 40.0, 48.0, 56.0, 64.0] {
            ...
            assert_eq!(
                ...
                result
                    .clusters
                    .iter()
                    .find(|cluster| cluster.range == text_range(2, 3))
                    .unwrap()
                    .advance
            );
            let line_texts: Vec<_> = result
                .lines
                .iter()
                .map(|line| Text::from(text).slice_text(line.range).to_string())
                .collect();
            assert!(
                line_texts
                    .iter()
            ...
            assert!(
                result
                    ...
                    .contextual_kinsoku_decisions
                    .iter()
                    .any(|decision| decision.source_text == "，"
                        && decision.reason
            ...
            assert!(
                result
                    ...
                    .all(|allocation| allocation.cluster_range != text_range(1, 2)
                        || allocation.kind != "InlineObjectBoundary")
            );
        }
    }
}
...
fn relation_stretch_moves_both_formula_sides_by_the_same_final_geometry() {
    ...
    let formula: Vec<_> = positioned
        .iter()
        .filter(|cluster| cluster.range.end() <= scalar_offset(3))
        .collect();
    ...
    let allocations: Vec<_> = result.debug.justification_decisions[0]
        .allocations
        .iter()
        .filter(|allocation| allocation.kind == "InlineObjectRelation")
        .collect();
...
fn formula_break_keeps_baseline_operator_on_previous_line() {
    ...
    let operator = InlineObjectSpan::new(
        ...
        InlineObjectBoundaryAdjustment::builder()
            .shrink_capacity(4.0)
            .line_end_discardable_advance(4.0)
            .build(),
    );
    for strategy in 0..3 {
        let objects = vec![
        ...
        assert_eq!(
            0.0,
            tiqian::core::layout_queries::positioned_clusters(&result)
                .iter()
                .find(|cluster| cluster.range == text_range(2, 3))
                .unwrap()
                .draw_x
        );
        assert!(result.debug.line_edge_trim_decisions.iter().any(
            |decision| decision.cluster_range == text_range(1, 2)
                && decision.reason == "InlineObjectLineEndDiscardableGlue"
                && decision.natural_glue == 4.0
        ));
        let decision = result
            .debug
            .inline_object_decisions
            .iter()
            .find(|decision| decision.range == text_range(1, 2))
            .unwrap();
