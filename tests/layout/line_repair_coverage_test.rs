...
fn push_in_fits_without_shrink_when_the_merged_line_already_matches() {
    ...
    let RepairOption::PushIn {
        ...
        reason,
        ..
    } = solution.lines[0].repair.as_ref().unwrap()
    else {
        panic!()
    };
...
fn push_in_filters_out_of_range_zero_capacity_and_foreign_line_end_only_opportunities() {
    ...
    let result = try_push_in(
        ...
        &[
            ...
            ShrinkOpportunity::with_line_end_only(
                4,
                4,
                16.0,
    ...
    let RepairOption::PushIn {
        ...
        ..
    } = result.previous.repair.as_ref().unwrap()
    else {
        panic!()
    };
...
fn push_in_reports_infinity_capacity_with_a_portable_debug_string() {
    ...
    let RepairOption::PushIn { reason, .. } = result.previous.repair.as_ref().unwrap() else {
        panic!()
    };
    assert_eq!(
        "ForbiddenAtLineStart:，:pushed-in=Infinity.0/Infinity.0",
        reason
    );
}
...
fn push_in_underflow_shares_skip_zero_valued_proportional_shares() {
    ...
    let natural = vec![
        cluster("a", 0, min_denormal, "a"),
        cluster("b", 1, min_denormal, "b"),
    ];
    let result = try_push_in(
        &line(
            IntRange::new(0, 0),
    ...
    assert!(result.candidate.accepted);
    let RepairOption::PushIn { allocations, .. } = result.previous.repair.as_ref().unwrap() else {
        panic!()
    };
    assert_eq!(1, allocations.len());
...
fn hang_merges_the_offender_beyond_the_measure() {
    ...
    let RepairOption::Hang {
        penalty,
        offender_cluster_index,
        ..
    } = merged.repair.as_ref().unwrap()
    else {
        panic!()
    };
...
fn hang_stops_before_a_non_zero_width_mandatory_break_tail() {
    ...
    assert_eq!(
        HashSet::from([4]),
        solution.lines[0].hanging_cluster_indices
    );
...
fn contextual_hang_extends_only_inside_its_protected_group() {
    ...
    assert_eq!(
        HashSet::from([3, 4]),
        extended.lines[0].hanging_cluster_indices
    );
    ...
    let RepairOption::CarryPrevious {
        carried_cluster_index,
        ..
    } = outside.lines[1].repair.as_ref().unwrap()
    else {
        panic!()
    };
    ...
    assert!(matches!(
        partial.lines[1].repair,
        Some(RepairOption::CarryPrevious { .. })
    ));

    let gap_natural = [
        clusters("AAA，", 0),
        vec![cluster("中", 4, EM, "中"), cluster("，", 5, EM, "，")],
        clusters("BB", 6),
    ]
    .concat();
...
fn leave_ragged_records_no_room_to_carry_for_a_single_cluster_line() {
    ...
    let RepairOption::LeaveRagged { reason, .. } = current.repair.as_ref().unwrap() else {
        panic!()
    };
...
fn leave_ragged_refuses_carries_that_would_split_an_unbreakable_span() {
    ...
    let carry = current
        ...
        .unwrap();
    assert_eq!(
        Some("carry-would-split-mourning-span"),
        carry.rejection_reason.as_deref()
    );
...
fn carry_previous_moves_the_previous_tail_down_when_it_fits() {
    ...
    let RepairOption::CarryPrevious {
        ...
        reason,
        ..
    } = solution.lines[1].repair.as_ref().unwrap()
    else {
        panic!()
    };
    assert_eq!(3, *carried_cluster_index);
    ...
    assert!(
        solution.lines[1]
            .repair_candidates
            .iter()
            .any(|candidate| candidate.kind == "CarryPrevious"
                && candidate.accepted
                && candidate.carried_cluster_index == Some(3))
    );
}
...
fn default_arguments_run_the_full_ragged_chain() {
    ...
    let RepairOption::LeaveRagged { penalty, .. } = current.repair.as_ref().unwrap() else {
        panic!()
    };
...
fn fill_push_in_default_arguments_omit_the_optional_boundaries() {
    ...
    assert_eq!(
        vec![IntRange::new(0, 4), IntRange::new(5, 7)],
        result
            .into_iter()
...
fn fill_push_in_extends_past_forbidden_heads_and_unbreakable_chains() {
    ...
    let chained = fill(
        ...
        None,
        &HashSet::new(),
        &[IntRange::new(4, 5), IntRange::new(5, 6)],
        &HashSet::new(),
        &HashMap::new(),
    );
...
fn fill_push_in_accepts_compression_denser_than_the_cured_stretch() {
    ...
    let RepairOption::PushIn { total_shrink, .. } = result[0].repair.as_ref().unwrap() else {
        panic!()
    };
...
fn fill_push_in_honours_progressive_tier_promotion_boundaries() {
    ...
    let promoted = fill(
        ...
        &[],
        &HashSet::new(),
        &HashMap::from([(2, emergency), (3, whitespace)]),
    );
    let RepairOption::PushIn { reason, .. } = promoted[0].repair.as_ref().unwrap() else {
        panic!()
    };
    assert_eq!(
        "ProgressiveTechnicalTierPromotion",
        reason.split(':').next().unwrap()
    );
    let degraded = fill(
        &lines,
        &natural,
        ...
        &[],
        &HashSet::new(),
        &HashMap::from([(2, whitespace), (3, emergency)]),
    );
    ...
    let empty_search = fill(
        ...
        &[],
        &HashSet::new(),
        &HashMap::from([(2, emergency), (4, whitespace)]),
    );
    ...
    let RepairOption::PushIn { reason, .. } = refill[0].repair.as_ref().unwrap() else {
        panic!()
    };
...
fn with_fill_push_in_gate_applies_or_returns_the_solution() {
    ...
    let disabled = with_fill_push_in(
        solution.clone(),
        false,
        &natural,
        &natural,
...
fn fill_pull_across_different_technical_spans_skips_tier_comparisons() {
    ...
    let result = fill(
        ...
        &HashMap::from([
            ...
            (
                6,
                ProgressiveBreakOpportunity::new(ProgressiveBreakTier::Syllable, text_range(4, 8)),
            ),
        ]),
    );
    assert_eq!(IntRange::new(0, 5), result[0].cluster_range);
