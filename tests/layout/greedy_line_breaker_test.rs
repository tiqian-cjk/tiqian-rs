...
fn fills_line_until_overflow_then_starts_new_line() {
    let clusters: Vec<_> = (0..5)
        .map(|index| cluster(index, index + 1, "x", 16.0))
        .collect();
...
fn kinsoku_carries_previous_when_push_in_capacity_cannot_cover_overflow() {
    ...
    assert_eq!(
        ...
        solution.lines[1].repair_candidates[0].rejection_reason
    );
...
fn leave_ragged_when_carry_previous_would_overflow() {
    ...
    assert_eq!(
        Some("carry-overflows".to_owned()),
        solution.lines[1].repair_candidates[1].rejection_reason
    );
...
fn retreats_break_so_line_does_not_end_on_opening_mark() {
    ...
    assert!(matches!(
        solution.lines[0].repair,
        Some(RepairOption::CarryNext {
            moved_cluster_index: 2,
            ..
        })
    ));
}
