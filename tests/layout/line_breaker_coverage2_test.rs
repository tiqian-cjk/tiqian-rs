...
fn test_lookahead_hard_break_at_end_and_middle() {
    ...
    let end_solution = LookaheadLineBreaker::default().break_lines(
        &end_clusters,
        &end_clusters,
        20.0,
        &end_config,
    ...
    let middle_solution = LookaheadLineBreaker::default().break_lines(
        &middle_clusters,
        &middle_clusters,
        20.0,
        &middle_config,
    ...
    assert_eq!(
        LineEndReason::MandatoryBreak,
        middle_solution.lines[0].end_reason
    );
    ...
    assert_eq!(
        ...
        LookaheadLineBreaker::default()
            .break_lines(&oversized, &oversized, 20.0, &oversized_config)
            .lines
            .len()
    );
}
...
fn test_line_candidate_ends_with_progressive_break() {
    ...
    assert!(!ends_with_progressive_break(
...
fn test_find_greedy_end_default_args() {
    ...
    assert_eq!(
        2,
        find_greedy_end(&clusters, 0, 25.0, clusters.len() as i32, &HashSet::new())
    );
}
