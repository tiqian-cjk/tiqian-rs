...
fn role_signature(text: &str) -> String {
    ...
    text.chars()
        .filter_map(|character| {
            ...
            utf16_index += character.len_utf16() as i32;
            matches!(character, '‘' | '’' | '“' | '”').then(|| {
                match roles.iter().find(|decision| decision.index == index).unwrap().role {
...
fn matches_single_quote_pair() {
    assert_eq!(
        vec![QuotePair::new(2, 5, QuoteType::Single)],
        analyze("他说‘你好’"),
    );
}
...
fn matches_nested_quote_pairs() {
    ...
    assert!(pairs.contains(&QuotePair::new(6, 9, QuoteType::Single)));
    assert!(pairs.contains(&QuotePair::new(3, 11, QuoteType::Double)));
}
...
fn contraction_apostrophe_does_not_close_outer_single_quote() {
    let text = "‘that’s’";
    assert_eq!(
        vec![QuotePair::new(0, text.encode_utf16().count() as i32 - 1, QuoteType::Single)],
        analyze(text),
    );
}
...
fn contraction_inside_cjk_single_quotes_keeps_apostrophe_latin() {
    ...
    assert_eq!(FontRole::CjkPunctuation, roles.iter().find(|decision| decision.index == 1).unwrap().role);
    assert_eq!(FontRole::CjkPunctuation, roles.iter().find(|decision| decision.index == 8).unwrap().role);
    assert_eq!(FontRole::LatinText, roles.iter().find(|decision| decision.index == 6).unwrap().role);
    assert_eq!(FontRole::LatinText, classifier.classify(&Text::from(text), TextRange::new(6, 7), &context));
}
...
fn in_word_apostrophe_matrix_does_not_consume_outer_quote_pairs() {
    for word in ["that’s", "l’été", "rock’n’roll", "version2’s", "α’β", "а’б", "e\u{0301}’s"] {
        ...
        assert_eq!(
            vec![QuotePair::new(0, quoted.encode_utf16().count() as i32 - 1, QuoteType::Single)],
            analyze(&quoted),
            "{quoted}",
        );
...
fn classifies_pair_as_cjk_when_outer_context_is_cjk() {
    let text = "他说“你好”";
    let roles = classify(text);
    assert_eq!(FontRole::CjkPunctuation, roles.iter().find(|decision| decision.index == 2).unwrap().role);
    assert_eq!(FontRole::CjkPunctuation, roles.iter().find(|decision| decision.index == 5).unwrap().role);
}

#[test]
fn classifies_pair_as_latin_when_outer_context_is_latin() {
    let roles = classify("he said “hello” world");
    assert_eq!(FontRole::LatinText, roles.iter().find(|decision| decision.index == 8).unwrap().role);
    assert_eq!(FontRole::LatinText, roles.iter().find(|decision| decision.index == 14).unwrap().role);
}
...
fn classifies_both_quotes_as_cjk_for_cjk_quoted_latin_content() {
    ...
    let roles = classify(text);
    assert_eq!(FontRole::CjkPunctuation, roles.iter().find(|decision| decision.index == 2).unwrap().role);
    assert_eq!(FontRole::CjkPunctuation, roles.iter().find(|decision| decision.index == text.encode_utf16().count() as i32 - 1).unwrap().role);
}

#[test]
fn whitespace_delimited_latin_quote_pair_overrides_cjk_outer_context() {
    let decisions = classify("（如 ‘O’, ‘Q’）");
    assert_eq!(vec![3, 5, 8, 10], decisions.iter().map(|decision| decision.index).collect::<Vec<_>>());
...
fn adjacent_quoted_list_items_do_not_use_previous_item_content_as_outer_context() {
    for (text, expected_signature) in [
        ...
        assert_eq!(expected_signature, role_signature(text), "{text}");
        let final_open = text[..text.rfind('“').unwrap()].encode_utf16().count() as i32;
        let final_close = text[..text.rfind('”').unwrap()].encode_utf16().count() as i32;
        let final_pair_decisions: Vec<_> = classify(text)
            .into_iter()
            .filter(|decision| decision.index == final_open || decision.index == final_close)
            .collect();
...
fn spaced_cjk_quoted_content_remains_cjk() {
    let text = "他说 ‘你好’";
    let roles = classify(text);
    assert_eq!(FontRole::CjkPunctuation, roles.iter().find(|decision| decision.index == 3).unwrap().role);
    assert_eq!(FontRole::CjkPunctuation, roles.iter().find(|decision| decision.index == text.encode_utf16().count() as i32 - 1).unwrap().role);
}
...
fn classifies_pair_as_cjk_at_text_boundary() {
    ...
    assert_eq!(FontRole::CjkPunctuation, roles.iter().find(|decision| decision.index == 0).unwrap().role);
    assert_eq!(FontRole::CjkPunctuation, roles.iter().find(|decision| decision.index == 3).unwrap().role);
}
...
fn classifies_text_start_latin_pair_from_quoted_content() {
    ...
    assert_eq!(FontRole::LatinText, roles.iter().find(|decision| decision.index == 0).unwrap().role);
    assert_eq!(FontRole::LatinText, roles.iter().find(|decision| decision.index == 6).unwrap().role);
}
...
fn mixed_chinese_question_at_paragraph_start_uses_paragraph_language() {
    ...
    assert_eq!(vec![0, text.encode_utf16().count() as i32 - 1], decisions.iter().map(|decision| decision.index).collect::<Vec<_>>());
...
fn numbered_cjk_quote_prefix_uses_quoted_content() {
    ...
    let decisions = classify(text);
    assert_eq!(FontRole::CjkPunctuation, decisions.iter().find(|decision| decision.index == 2).unwrap().role);
    assert_eq!(FontRole::CjkPunctuation, decisions.iter().find(|decision| decision.index == text.encode_utf16().count() as i32 - 1).unwrap().role);
    assert_eq!("PairedPunctuationContentScriptContext", decisions.iter().find(|decision| decision.index == 2).unwrap().source);
}

#[test]
fn numbered_latin_quote_prefix_still_uses_latin_content() {
    let roles = classify("1.“Hello”");
    assert_eq!(FontRole::LatinText, roles.iter().find(|decision| decision.index == 2).unwrap().role);
    assert_eq!(FontRole::LatinText, roles.iter().find(|decision| decision.index == 8).unwrap().role);
}
...
fn classifies_nested_pairs_by_outermost_context() {
    ...
    for index in [3, 11, 6, 9] {
        assert_eq!(FontRole::CjkPunctuation, roles.iter().find(|decision| decision.index == index).unwrap().role);
    }
}
...
fn skips_ascii_punctuation_when_resolving_context() {
    ...
    assert_eq!(FontRole::LatinText, roles.iter().find(|decision| decision.index == 9).unwrap().role);
    assert_eq!(FontRole::LatinText, roles.iter().find(|decision| decision.index == 15).unwrap().role);
}

#[test]
fn skips_neutral_dash_when_resolving_context() {
    let roles = classify("English — “hello”");
    assert_eq!(FontRole::LatinText, roles.iter().find(|decision| decision.index == 10).unwrap().role);
    assert_eq!(FontRole::LatinText, roles.iter().find(|decision| decision.index == 16).unwrap().role);
}
