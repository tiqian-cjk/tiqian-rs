...
fn classify(text: &str) -> Vec<tiqian::layout::quote_pair_analyzer::QuoteRoleDecision> {
    ...
    analyzer.classify_quote_roles(&text, &analyzer.analyze(&text), &FontRoleContext::default())
}
    ...
    text.chars()
        .filter_map(|character| {
            ...
            matches!(character, '‘' | '’' | '“' | '”').then(|| {
                match roles
                    .iter()
                    .find(|decision| decision.index.value() == index)
                    .unwrap()
                    .role
                {
...
fn matches_nested_quote_pairs() {
    ...
    assert!(pairs.contains(&QuotePair::new(
        scalar_offset(6),
        scalar_offset(9),
        QuoteType::Single
    )));
...
fn contraction_inside_cjk_single_quotes_keeps_apostrophe_latin() {
    ...
    assert_eq!(
        ...
        roles
            .iter()
            .find(|decision| decision.index.value() == 8)
            .unwrap()
            .role
    );
    assert_eq!(
    ...
    assert_eq!(
        FontRole::LatinText,
        classifier.classify(&Text::from(text), text_range(6, 7), &context)
    );
}
...
fn in_word_apostrophe_matrix_does_not_consume_outer_quote_pairs() {
    for word in [
        ...
        assert_eq!(
            vec![QuotePair::new(
                scalar_offset(0),
                scalar_offset(quoted.chars().count() as i32 - 1),
                QuoteType::Single
            )],
            analyze(&quoted),
            "{quoted}",
...
fn classifies_pair_as_cjk_when_outer_context_is_cjk() {
    ...
    assert_eq!(
        ...
        roles
            .iter()
            .find(|decision| decision.index.value() == 5)
            .unwrap()
            .role
    );
}
...
fn classifies_pair_as_latin_when_outer_context_is_latin() {
    ...
    assert_eq!(
        ...
        roles
            .iter()
            .find(|decision| decision.index.value() == 14)
            .unwrap()
            .role
    );
}
...
fn classifies_both_quotes_as_cjk_for_cjk_quoted_latin_content() {
    ...
    assert_eq!(
        ...
        roles
            .iter()
            .find(|decision| decision.index.value() == text.chars().count() as i32 - 1)
            .unwrap()
            .role
    );
}
...
fn adjacent_quoted_list_items_do_not_use_previous_item_content_as_outer_context() {
    for (text, expected_signature) in [
        ...
        let final_pair_decisions: Vec<_> = classify(text)
            .into_iter()
            .filter(|decision| {
                decision.index.value() == final_open || decision.index.value() == final_close
            })
            .collect();
        assert_eq!(2, final_pair_decisions.len(), "{text}");
        assert!(
            final_pair_decisions
                .iter()
                .all(|decision| decision.source == "PairedPunctuationOuterScriptContext"),
            "{text}: {final_pair_decisions:?}"
        );
    }
}
...
fn skips_ascii_punctuation_when_resolving_context() {
    ...
    assert_eq!(
        ...
        roles
            .iter()
            .find(|decision| decision.index.value() == 15)
            .unwrap()
            .role
    );
}
...
fn skips_neutral_dash_when_resolving_context() {
    ...
    assert_eq!(
        ...
        roles
            .iter()
            .find(|decision| decision.index.value() == 10)
            .unwrap()
            .role
    );
    assert_eq!(
        ...
        roles
            .iter()
            .find(|decision| decision.index.value() == 16)
            .unwrap()
            .role
    );
}
...
fn representative_quote_context_matrix_remains_stable() {
    for (text, expected_signature) in [
        ...
        ("1.“中文”", "CC"),
        ("1.“Hello”", "LL"),
        ("他说：“She said ‘hello’.”", "CLLC"),
        ("English “他说‘你好’” end", "LCCL"),
        ("中文‘don’t’", "CLC"),
        ("中文 ‘don’t’", "LLL"),
        ("他说：“第一行\n第二行。”", "CC"),
        ("（如\t‘O’）", "LL"),
...
fn role_decision_sources_stay_explainable_across_fallback_paths() {
    for (text, expected_source) in [
        ...
        assert!(
            decisions
                .iter()
                .all(|decision| decision.source == expected_source),
            "{text}: {decisions:?}"
        );
    }
}
