...
fn scalar_index_of(text: &str, needle: char) -> ScalarOffset {
    scalar_offset(
        text.find(needle)
            .map(|byte_index| text[..byte_index].chars().count() as i32)
            .unwrap(),
    )
}
...
fn classifies_ascii_brackets_as_latin_regardless_of_surrounding_context() {
    ...
    assert_eq!(
        ...
        result
            ...
            .font_decisions
            .iter()
            .find(|decision| decision.source_text == "(English)")
            .unwrap()
            .role
    );
}
...
fn ascii_closing_bracket_with_cjk_interior_is_forbidden_at_line_start() {
    ...
    let result = engine.layout(
        LayoutInput::builder(
            TiqianTextContent::new(Text::from(text)),
            LayoutConstraints::with_defaults(232.0),
        )
        .paragraph_style(
            ParagraphStyle::builder()
                .first_line_indent(Some(Ic::ZERO))
    ...
    assert!(
        result
            .lines
            .iter()
            .all(|line| !source.slice_text(line.range).as_str().starts_with(')')),
        "{:#?}",
        result.lines
    );
    assert_eq!(
...
fn ascii_opening_bracket_with_cjk_interior_is_forbidden_at_line_end() {
    ...
    let result = engine.layout(
        LayoutInput::builder(
            TiqianTextContent::new(Text::from(text)),
            LayoutConstraints::with_defaults(168.0),
        )
        .paragraph_style(
            ParagraphStyle::builder()
                .first_line_indent(Some(Ic::ZERO))
    ...
    assert!(
        result
            .lines
            .iter()
            .all(|line| !source.slice_text(line.range).as_str().ends_with('(')),
        "{:#?}",
        result.lines
    );
    assert_eq!(
...
fn keeps_text_start_latin_quote_pair_in_latin_run() {
    ...
    assert!(
        result
            ...
            .font_decisions
            .iter()
            .any(|decision| decision.range == text_range(0, 7) && decision.role == "LatinText")
    );
}
...
fn mixed_quote_contexts_reach_the_font_and_punctuation_pipeline() {
    ...
    for index in &cjk_quote_indices {
        assert_eq!(
            ...
            result
                ...
                .font_decisions
                .iter()
                .find(|decision| *index >= decision.range.start() && *index < decision.range.end())
                .unwrap()
                .role
        );
    }
    ...
    let overrides: std::collections::HashMap<_, _> = result
        .debug
        .role_overrides
        .iter()
        .map(|override_info| {
    ...
    for (index, role) in overrides {
        assert_eq!(
            if cjk_quote_indices.contains(&index) {
                "CjkPunctuation"
            } else {
                "LatinText"
            },
...
fn quote_roles_survive_style_and_source_boundaries() {
    ...
    let roles: std::collections::HashMap<_, _> = result
        .debug
        .role_overrides
        .iter()
        .map(|override_info| {
    ...
    for text in [
        ...
        assert_eq!(
            ...
            result
                ...
                .font_decisions
                .iter()
                .filter(|decision| decision.role == "CjkPunctuation"
        ...
        let final_overrides: Vec<_> = result
            .debug
            .role_overrides
            .iter()
            .filter(|override_info| final_indices.contains(&override_info.range.start()))
            .collect();
...
fn keeps_slash_led_latin_technical_run_out_of_cjk_punctuation_geometry() {
    ...
    let latin_run = result
        .debug
        .font_decisions
        .iter()
        .find(|decision| decision.source_text == "/TERFism")
        .unwrap();
    ...
    assert!(
        result
            ...
            .punctuation_decisions
            .iter()
            .all(|decision| decision.range != latin_run.range)
    );
    let cluster = result
        .clusters
        .iter()
...
fn mixed_chinese_question_at_paragraph_start_keeps_cjk_quote_geometry() {
    ...
    let quote_indices: HashSet<ScalarOffset> = HashSet::from([
        scalar_offset(0),
        scalar_offset(text.chars().count() as i32 - 1),
    ]);
    let overrides: Vec<_> = result
        .debug
        .role_overrides
        .iter()
        .filter(|override_info| quote_indices.contains(&override_info.range.start()))
        .collect();
    assert_eq!(
        quote_indices,
        overrides
            .iter()
            .map(|override_info| override_info.range.start())
            .collect()
    );
    assert!(
        overrides
...
fn keeps_numbered_cjk_quote_pair_on_cjk_face() {
    ...
    assert_eq!(
        ...
        opening
            .resolved_face
            .as_ref()
            .expect("font decision must have a resolved font face")
            .resource_id()
    );
...
fn leaves_latin_context_curly_quotes_outside_cjk_punctuation_geometry() {
    ...
    assert!(
        result
            .clusters
            .iter()
            .all(|cluster| cluster.glyph_inline_shift == 0.0)
    );
}
...
fn keeps_contraction_apostrophe_latin_inside_cjk_single_quotes() {
    ...
    let contraction = result
        ...
        .font_decisions
        .iter()
        .find(|decision| decision.range == text_range(2, 8))
        .unwrap();
    let closing = result
        .debug
        .font_decisions
        .iter()
        .find(|decision| decision.range == text_range(8, 9))
        .unwrap();
    assert_eq!("CjkPunctuation", opening.role);
    ...
    assert_eq!(
        ...
        result
            .clusters
            .iter()
            .find(|cluster| cluster.text == "that’s")
            .unwrap()
            .font_face
            .as_ref()
            .expect("cluster must have a final font face")
    ...
    assert!(
        result
            ...
            .punctuation_decisions
            .iter()
            .all(|decision| decision.range != text_range(6, 7))
    );
}
...
fn keeps_digit_bounded_word_internal_quotes_cjk() {
    ...
    assert!(
        overrides.iter().all(
            |override_info| override_info.overridden_role == "CjkPunctuation"
