...
use tiqian::clreq::clreq_profile::{
    ClreqProfile, ClreqProfileResolver, GlueSide, InteriorPunctuationStyle, PunctuationClass,
...
fn punctuation_glue(natural: f32) -> Glue {
    Glue::new(GlueKind::PunctuationTrailing, 0.0, natural, natural, 0, 0)
}

fn advance_of_mid_line_punctuation(
    text: &str,
    punctuation: &str,
    width: PunctuationWidthPolicy,
) -> f32 {
...
fn compression_result_sums_adjustment_reductions() {
    ...
    assert_eq!(
        0.0,
        PunctuationSpacingCompressionResult::new(Vec::new()).total_reduction()
    );
}
...
fn adjacent_punctuation_inner_glue_collapses_by_half_em() {
    ...
    assert_eq!(
        "collapse-adjacent-punctuation-inner-glue",
        adjustment.reason
    );
...
fn cjk_closing_before_ascii_point_mark_collapses_trailing_glue() {
    ...
    let adjustment = PunctuationSpacingCompressor
        .compress_cjk_closing_before_ascii_point_mark(
            &[closing.clone()],
            &Text::from("」, rest"),
            EM,
        )
...
fn indexed_build_rejects_out_of_range_index() {
    ...
    assert_eq!(
        None,
        builder.build_at(&Text::from("，"), scalar_offset(5), EM)
    );
    let built = builder
        .build_at(&Text::from("，"), scalar_offset(0), EM)
        .unwrap();
...
fn non_punctuation_characters_produce_no_atom() {
    assert_eq!(
        None,
        PunctuationAtomBuilder::default().build_at(&Text::from("中"), scalar_offset(0), EM)
    );
    assert_eq!(
        None,
        PunctuationAtomBuilder::default().build_at(&Text::from("a"), scalar_offset(0), EM)
    );
}
...
fn kaiming_style_halves_interior_punctuation_but_not_sentence_end() {
    ...
    assert_eq!(
        16.0,
        advance_of_mid_line_punctuation("中，中文", "，", full)
    );
    assert_eq!(
        8.0,
        advance_of_mid_line_punctuation("中，中文", "，", kaiming)
    );
    assert_eq!(
        8.0,
        advance_of_mid_line_punctuation("中（中）文", "（", kaiming)
    );
    assert_eq!(
        16.0,
        advance_of_mid_line_punctuation("中。中文", "。", kaiming)
...
fn underwidth_glyphs_expand_into_full_width_cell_by_class_side() {
    ...
    assert_eq!(
        ...
        opening.glyph_placement_reason
    );
...
fn halt_advance_without_placement_falls_back_to_fitted_ink_or_profile() {
    ...
    assert_eq!(
        ...
        with_ink.geometry_source
    );
    let without_ink = atom(
        '，',
        0,
        Some(
            PunctuationInkInput::builder(16.0)
...
fn halt_from_proportional_glyph_is_rejected() {
    let result = atom(
        ...
        0,
        Some(
            PunctuationInkInput::builder(8.0)
                .halt_advance(Some(4.0))
                .halt_placement_x(Some(-2.0))
                .build(),
        ),
    );
...
fn ink_input_records_why_bounds_are_missing() {
    let no_ink = atom(
        ...
        Some(
            PunctuationInkInput::builder(16.0)
                .bounds_fallback_reason(Some("shaper-no-ink-bounds".to_owned()))
                .build(),
        ),
    );
    assert_eq!(
        Some("shaper-no-ink-bounds".to_owned()),
        no_ink.ink_bounds_fallback
    );
    ...
    let ambiguous = atom(
        ...
        0,
        Some(
            PunctuationInkInput::builder(0.0)
...
fn glue_side_for_mainland_simplified_maps_classes_to_sides() {
    ...
    assert_eq!(
        ...
        PunctuationGluePlacement::MainlandSimplified.glue_side_for(PunctuationClass::Closing)
    );
    assert_eq!(
        GlueSide::TrailingOnly,
        PunctuationGluePlacement::MainlandSimplified.glue_side_for(PunctuationClass::PauseOrStop)
    );
    assert_eq!(
        GlueSide::BothSides,
        PunctuationGluePlacement::MainlandSimplified.glue_side_for(PunctuationClass::MiddleDot)
    );
    assert_eq!(
        GlueSide::BothSides,
        PunctuationGluePlacement::Traditional.glue_side_for(PunctuationClass::Opening)
    );
}
