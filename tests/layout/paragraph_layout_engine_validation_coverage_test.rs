...
fn expect_layout_continues(layout_input: LayoutInput) {
    ...
    assert!(
        result
            .clusters
            .iter()
            .all(|cluster| cluster.advance.is_finite())
    );
...
fn overlapping_inline_object_ranges_keep_the_earlier_source_start() {
    ...
    assert_eq!(
        text_range(0, 2),
        result.debug.inline_object_decisions[0].range
    );
    assert_eq!(10.0, result.debug.inline_object_decisions[0].advance);
}
...
fn invalid_inline_object_leading_boundary_uses_fixed_boundary() {
    let shrink = InlineObjectBoundaryAdjustment::builder()
        .shrink_capacity(0.5)
        .build();
    expect_layout_continues(input(
        ParagraphStyle::default(),
...
fn invalid_inline_object_preferred_stretch_uses_zero_capacity() {
    ...
    assert_eq!(
        0.0,
        result.debug.inline_object_decisions[0].trailing_preferred_stretch_capacity
    );
    let next = engine.layout(input(
        ParagraphStyle::default(),
        Vec::new(),
