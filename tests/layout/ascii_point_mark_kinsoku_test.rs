...
fn cjk_attached_ascii_point_marks_cannot_start_wrapped_lines_and_stay_latin() {
    for mark in [',', '.', ':', ';', '!', '?'] {
        for breaker in greedy_and_lookahead() {
            ...
            assert!(
                line_texts(&result, &text)
                    .iter()
                    .all(|line| !line.starts_with(mark))
            );
            let point = result
                .clusters
                .iter()
                .find(|cluster| cluster.text == mark.to_string())
                .unwrap();
            assert_eq!(
                "latin-primary",
                point
                    .font_face
                    .as_ref()
                    .expect("point must have a final font face")
                    .resource_id()
            );
            assert_eq!(
                "LatinText",
                result
                    ...
                    .font_decisions
                    .iter()
                    .find(|decision| decision.range == point.range)
                    .unwrap()
                    .role
            );
            assert!(
                result
                    ...
                    .iter()
                    .all(|decision| decision.range != point.range)
            );
            let decision = result
                .debug
...
fn latin_tokens_and_ambiguous_ascii_characters_keep_existing_segmentation() {
    ...
    let positions: std::collections::HashSet<_> = result
        .debug
        .contextual_kinsoku_decisions
        .iter()
        .filter(|decision| decision.source_text == "\"quoted\"")
        .map(|decision| decision.forbidden_position.as_str())
        .collect();
    assert_eq!(
        std::collections::HashSet::from(["LineStart", "LineEnd"]),
        positions
    );
}
    ...
    for width in [1.0, 8.0, 15.0, 23.0, 31.0] {
        for breaker in greedy_and_lookahead() {
            ...
            assert_eq!(
                ...
                result.debug.contextual_kinsoku_decisions[0]
            ...
            assert!(
                result
                    ...
                    .line_decisions
                    .iter()
                    .any(|decision| decision.repair.as_deref() == Some("Hang"))
            );
        }
    }
}
    ...
    for breaker in greedy_and_lookahead() {
        ...
        assert_eq!(
            ...
            result
                ...
                .contextual_kinsoku_decisions
                .iter()
                .filter(|decision| decision.impossible_measure_fallback.as_deref()
                    == Some("AttachedAsciiPointMarkImpossibleMeasureHang"))
                .count()
        );
        let hanging = result
            .lines
        ...
        let expected: f32 = result
            .clusters
            .iter()
            .filter(|cluster| cluster.text == "!" || cluster.text == ",")
    ...
    for breaker in greedy_and_lookahead() {
        ...
        assert_eq!(
            ...
            result
                .lines
                .iter()
                .filter(|line| line.hanging_punctuation_advance > 0.0)
                .count()
        );
        assert!(
            line_texts(&result, text)
