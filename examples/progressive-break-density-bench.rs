...
impl Options {
    fn parse() -> Result<Option<Self>, String> {
        ...
        while let Some(flag) = args.next() {
            if flag == "--help" || flag == "-h" {
                println!(
                    "progressive-break-density-bench [--length 8192] [--iterations 200] [--warmup 20] [--calls-per-sample 100]"
                );
                return Ok(None);
            }
            if !matches!(
...
impl Fixture {
    fn new(length: usize, terminal_span_covers_line: bool) -> Self {
        ...
        let opportunities = tiers
            ...
            .enumerate()
            .map(|(index, tier)| {
                let boundary = line_end - (tiers.len() - 1 - index) as i32;
                let opportunity = if tier == ProgressiveBreakTier::Whitespace {
                    ProgressiveBreakOpportunity::with_preceding_whitespace_stretch_capacity(
                        tier, span, 0.25,
                    )
                } else {
                    ProgressiveBreakOpportunity::new(tier, span)
                };
                (boundary, opportunity)
            })
            .collect();
        let cjk_inter_char_boundaries = (1..line_end).collect();
        let sino_western_boundaries = (1..line_end).step_by(2).collect();
        Self {
            clusters,
            opportunities,
...
fn main() -> Result<(), String> {
    ...
    let scenarios = [
        (
            "terminal-technical-tracking",
            Fixture::new(options.length, true),
            options.length as f32 * 1.5,
        ),
        (
            "cjk-gap-fallback",
            Fixture::new(options.length, false),
            options.length as f32 * 1.5,
        ),
    ];
    ...
    for (label, fixture, line_limit) in scenarios {
        ...
        let samples: Vec<_> = (0..options.iterations)
            ...
            .collect();
        print_stats(
            &format!("decide_progressive_break[{label}]"),
            &samples,
            options.calls_per_sample,
        );
    }
    println!(
        "Measured calls: {}",
        options.iterations * options.calls_per_sample * scenario_count
    );
    Ok(())
}
