...
use super::super::clreq::clreq_profile::ClreqProfileResolver;
...
use super::progressive_break_decisions::{ProgressiveBreakTier, ShrinkChannel, ShrinkOpportunity};
use super::punctuation_geometry_ledger::{PunctuationGeometryLedger, cluster_index_range_for};
...
use super::quote_pair_analyzer::{QuotePair, QuotePairAnalyzer, QuotePairAwareFontRoleClassifier};
...
pub fn build_paragraph_layout_prep(
    ...
    let mut boundary: BTreeMap<
        i32,
        super::super::core::text_model::InlineObjectBoundaryAdjustment,
    > = BTreeMap::new();
    for (index, _) in natural.iter().enumerate() {
        ...
        for (left, adjustment) in [
            ...
            if *adjustment == super::super::core::text_model::InlineObjectBoundaryAdjustment::FIXED
            {
                continue;
            }
            if let Some(previous) = boundary.get(&left).cloned() {
                ...
                boundary.insert(
                    left,
                    super::super::core::text_model::InlineObjectBoundaryAdjustment::builder()
