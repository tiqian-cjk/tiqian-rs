...
use super::super::linebreak::unicode_punctuation_line_break::{
...
pub fn resolve_attached_inline_virtual_boundaries(
    inline_attachments: &[super::super::core::text_model::InlineAttachment],
...
fn has_authored_break(s: &Text) -> bool {
    s.chars().any(|c| {
        is_mandatory_break_code_point(c as i32) || is_zero_width_space_code_point(c as i32)
    })
}
fn previous_content_cluster(index: usize, clusters: &[Cluster], text: &Text) -> Option<i32> {
...
fn next_content_cluster(index: usize, clusters: &[Cluster], text: &Text) -> Option<i32> {
...
fn decimal_mark_after_space(index: usize, clusters: &[Cluster], text: &Text) -> bool {
    if index == 0 {
        return false;
    }
    ...
    let current = cluster_text(text, &clusters[index]).unwrap_or_default();
